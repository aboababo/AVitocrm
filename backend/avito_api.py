...
class AvitoAPI:
                ...
                try:
                    ...
                    if response.text:
                        if response.text.strip().startswith("<!"):
                            error_text = (
                                f"Получен HTML вместо JSON (возможно страница ошибки). Статус: {response.status_code}"
                            )
                        else:
    ...
    def _make_request(
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_listing_request and attempt > 0:
                    logger.info(
                        f"[AVITO API] Повторная попытка {attempt + 1}/{max_retries} для получения объявления..."
                    )
                ...
                if is_listing_request:
                    logger.info(
                        f"[AVITO API] Ответ для объявления: статус {response.status_code} за {attempt_elapsed:.2f} сек (попытка {attempt + 1})"
                    )
                else:
                    logger.debug(
                        f"[AVITO API] API ответ: {response.status_code} для {endpoint} (попытка {attempt + 1})"
                    )
                    ...
                    if response.content:
                        try:
                            ...
                            if is_listing_request:
                                ...
                                logger.info(
                                    f"[AVITO API] Объявление получено успешно: размер ответа {content_size} байт, "
                ...
                # Обработка 401 - токен истек
                    ...
                    if is_listing_request:
                        logger.warning("[AVITO API] Токен истек при запросе объявления, получаем новый...")
                    else:
                ...
                # Обработка 403 - Forbidden (нет доступа к ресурсу)
                    ...
                    try:
                        ...
                        error_message = error_data.get(
                            "message",
                            error_data.get("error", "Forbidden - нет доступа"),
                        )
                    except:
                    ...
                    if is_listing_request:
                        logger.error(f"[AVITO API] {error_msg}")
                        logger.error(
                            "[AVITO API] Возможно, объявление не принадлежит пользователю или нет прав доступа"
                        )
                    else:
                        logger.error(error_msg)
                    raise Exception(error_msg)
                ...
                # Обработка 404 - Not Found (endpoint не найден)
                    ...
                    try:
                        ...
                        error_message = error_data.get(
                            "message",
                            error_data.get("error", "Not Found - endpoint не найден"),
                        )
                    except:
                    ...
                    logger.error("[AVITO API] Возможные причины:")
                    logger.error("[AVITO API] 1. Endpoint не доступен для данного типа аккаунта")
                    logger.error("[AVITO API] 2. Требуется специальный тариф или права доступа")
                    logger.error(
                        "[AVITO API] 3. Проверьте документацию: https://developers.avito.ru/api-catalog/messenger/documentation"
                    )
                    raise Exception(error_msg)
                ...
                # Обработка 405 - Method Not Allowed (метод не разрешен)
                    ...
                    print(
                        f"[AVITO API] Response body (first 500 chars): {response.text[:500] if response.text else 'empty'}"
                    )
                    print("[AVITO API] ==================================")
                    print("=" * 80 + "\n")
                    # Также логируем в logger
                    logger.error("[AVITO API] ========== ОШИБКА 405 ==========")
                    logger.error(f"[AVITO API] {error_msg}")
                    logger.error(f"[AVITO API] Endpoint: {endpoint}")
                ...
                # Обработка 404 - Not Found (endpoint не найден)
                    ...
                    if method == "POST" and "/messages" in endpoint:
                        ...
                        print("=" * 80 + "\n")
                    # Также логируем в logger
                    logger.error("[AVITO API] ========== ОШИБКА 404 ==========")
                    logger.error(f"[AVITO API] {error_msg}")
                    logger.error(f"[AVITO API] Endpoint: {endpoint}")
                    logger.error(f"[AVITO API] Method: {method}")
                    logger.error(f"[AVITO API] Full URL: {url}")
                ...
                # Обработка 422 - Unprocessable Entity (ошибка валидации)
                    ...
                    if is_listing_request:
                        ...
                        logger.warning(
                            "[AVITO API] Возможно, объявление не принадлежит пользователю или item_id неверный"
                        )
                    else:
                        logger.error(error_msg)
                    raise Exception(error_msg)
                ...
                # Улучшенная retry логика с exponential backoff и jitter
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"Временная ошибка сервера {response.status_code}. Повтор через {wait_time:.2f} сек... (попытка {attempt + 1}/{max_retries})"
                        )
                ...
                # Обработка 408 - Request Timeout (может быть временной)
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"Request Timeout (408). Повтор через {wait_time:.2f} сек... (попытка {attempt + 1}/{max_retries})"
                        )
                        time.sleep(wait_time)
                        continue
                ...
                # Обработка 504 - Gateway Timeout (может быть временной)
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"Gateway Timeout (504). Повтор через {wait_time:.2f} сек... (попытка {attempt + 1}/{max_retries})"
                        )
                        time.sleep(wait_time)
                        continue
                ...
                if 400 <= response.status_code < 500 and response.status_code != 429:
                    ...
                    if is_listing_request:
                        logger.error(
                            f"[AVITO API] ОШИБКА при получении объявления (время {total_elapsed:.2f} сек): {error_msg}"
                        )
                ...
                if attempt < max_retries - 1:
                    ...
                    if is_listing_request:
                        logger.warning(
                            f"[AVITO API] Ошибка {response.status_code} при запросе объявления. "
                    ...
                    total_elapsed = time.time() - request_start_time
                    error_msg = (
                        f"Ошибка API запроса после {max_retries} попыток: {response.status_code} - {error_message}"
                    )

                    if is_listing_request:
                        logger.error(
                            f"[AVITO API] ОШИБКА при получении объявления после {max_retries} попыток "
                ...
                if attempt < max_retries - 1:
                    ...
                    if is_listing_request:
                        logger.warning(
                            f"[AVITO API] Timeout при запросе объявления (время {total_elapsed:.2f} сек). "
                            f"Повтор через {wait_time:.2f} сек... (попытка {attempt + 1}/{max_retries})"
                        )
                    else:
                    ...
                    if is_listing_request:
                        logger.error(
                            f"[AVITO API] Timeout после {max_retries} попыток при запросе объявления "
                ...
                if attempt < max_retries - 1:
                    ...
                    if is_listing_request:
                        logger.warning(
                            f"[AVITO API] Ошибка запроса объявления (время {total_elapsed:.2f} сек). "
                            f"Повтор через {wait_time:.2f} сек... (попытка {attempt + 1}/{max_retries})"
                        )
                    else:
                    ...
                    if is_listing_request:
                        logger.error(
                            f"[AVITO API] Ошибка сетевого запроса объявления после {max_retries} попыток "
                            f"(всего времени {total_elapsed:.2f} сек): {e}"
                        )
                        logger.error(f"[AVITO API] Endpoint: {endpoint}")
                    else:
        ...
        if last_exception:
            if is_listing_request:
                logger.error(
                    f"[AVITO API] Все попытки исчерпаны при запросе объявления "
        ...
        if is_listing_request:
            logger.error(
                f"[AVITO API] Не удалось выполнить запрос объявления после {max_retries} попыток "
    ...
    def get_chat_messages(self, user_id: str, chat_id: str, limit: int = 100, offset: int = 0) -> Dict:
        """
        ...
        if "~" in chat_id or "/" in chat_id or "=" in chat_id:
            ...
            if safe_encoded != chat_id and safe_encoded != encoded_chat_id:
                endpoints_to_try.insert(
                    1,
                    f"/messenger/v3/accounts/{user_id}/chats/{safe_encoded}/messages/",
                )
    ...
    def send_message(
        ...
        logger.info("[SEND MESSAGE] === НАЧАЛО ОТПРАВКИ ===")
        logger.info(
            f"[SEND MESSAGE] user_id={user_id}, chat_id={chat_id}, message_length={len(message) if message else 0}"
        )
        ...
        request_data_v1 = {"message": {"text": str(message)}, "type": "text"}

        # Формат для v3/v2 (fallback): {"message": "текст"}
        request_data_v3 = {"message": str(message)}
        if attachments:
        ...
        if encoded_chat_id != chat_id:
            endpoints_to_try.append(
                (
                    "v1",
                    f"/messenger/v1/accounts/{user_id}/chats/{encoded_chat_id}/messages",
                )
            )
        ...
        if "~" in chat_id or "/" in chat_id or "=" in chat_id:
            ...
            if safe_encoded != chat_id and safe_encoded != encoded_chat_id:
                endpoints_to_try.append(
                    (
                        "v1",
                        f"/messenger/v1/accounts/{user_id}/chats/{safe_encoded}/messages",
                    )
                )
        ...
        if encoded_chat_id != chat_id:
            endpoints_to_try.append(
                (
                    "v2",
                    f"/messenger/v2/accounts/{user_id}/chats/{encoded_chat_id}/messages",
                )
            )
        ...
        if last_error:
            ...
            print("[SEND MESSAGE] ==========================================")
            print("=" * 80 + "\n")
            logger.error("[SEND MESSAGE] ========== ВСЕ ПОПЫТКИ НЕУДАЧНЫ ==========")
    ...
    def get_listing(self, user_id: str, item_id: str, params: Optional[Dict] = None) -> Dict:
        ...
        """
        logger.info("[AVITO API] ========== НАЧАЛО ЗАГРУЗКИ ОБЪЯВЛЕНИЯ ==========")
        ...
        try:
            ...
            if isinstance(result, dict):
                ...
                logger.info(
                    f"[AVITO API] Объявление получено: id={listing_id}, title='{listing_title}', "
                ...
                if "price_info" in result:
                    logger.info(
                        f"[AVITO API] Поле 'price_info': {result.get('price_info')} (тип: {type(result.get('price_info'))})"
                    )
                if "title" in result:
            ...
            if isinstance(result, dict):
                logger.info("[AVITO API] ========== СТРУКТУРА ОТВЕТА API ==========")
                ...
                for key in [
                    ...
                    if key in result:
                        ...
                        if isinstance(value, (list, dict)):
                            logger.info(
                                f"[AVITO API] {key}: {type(value).__name__}, длина/размер: {len(value) if hasattr(value, '__len__') else 'N/A'}"
                            )
            ...
            logger.info("[AVITO API] ========== ЗАГРУЗКА ОБЪЯВЛЕНИЯ ЗАВЕРШЕНА УСПЕШНО ==========")
            ...
            request_elapsed = time.time() - request_start
            logger.error(
                f"[AVITO API] ОШИБКА при получении объявления за {request_elapsed:.2f} сек: {str(e)}",
                exc_info=True,
            )
            logger.error(
                f"[AVITO API] Параметры запроса: user_id='{user_id}', item_id='{item_id}', endpoint='{endpoint}'"
            )
            raise
    ...
    def get_listing_statistics(self, user_id: str, item_id: str, date_from: str, date_to: str) -> Dict:
        """
    ...
    def remove_from_blacklist(
        ...
        blacklist_entry_id: Optional[str] = None,
    ) -> bool:
        """
        ...
        if not data:
            raise ValueError(
                "Необходимо указать либо телефон (phone), либо user_id, либо blacklist_entry_id для разблокировки"
            )
    ...
    def update_webhook(self, user_id: str, webhook_id: str, url: str, events: List[str]) -> Dict:
        """
        ...
        with open(file_path, "rb") as f:
            ...
            try:
                ...
                if response.status_code == 200 or response.status_code == 201:
                    result = response.json()
                    logger.info(
                        f"Медиа-файл успешно загружен: {file_path}, attachment_id: {result.get('attachment_id') or result.get('id')}"
                    )
                    return result
                else:
    ...
    def upload_media_from_bytes(
        ...
        if response.status_code == 200 or response.status_code == 201:
            result = response.json()
            logger.info(
                f"Медиа-файл успешно загружен из байтов: {filename}, attachment_id: {result.get('attachment_id') or result.get('id')}"
            )
            return result
        else:
            error_msg = f"Ошибка загрузки медиа: {response.status_code} - {response.text}"
    ...
    def upload_images(self, user_id: str, image_paths: List[str]) -> List[Dict]:
        ...
        for image_path in image_paths:
            ...
            if ext not in supported_formats:
                raise ValueError(
                    f"Файл {image_path} не является изображением поддерживаемого формата (поддерживаются: JPEG, HEIC, GIF, BMP, PNG)"
                )
            ...
            if file_size > max_size:
                raise ValueError(
                    f"Файл {image_path} слишком большой: {file_size} байт. Максимум: {max_size} байт (24 МБ)"
                )
                ...
                try:
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        if isinstance(result, list):
                            ...
                            items = result.get(
                                "images",
                                result.get("items", result.get("data", [result])),
                            )
                        ...
                        error_msg = (
                            f"Ошибка загрузки изображения {image_path}: {response.status_code} - {response.text}"
                        )
                        logger.error(error_msg)
                        raise Exception(error_msg)
                except Exception as e:
    ...
    def upload_images_from_bytes(self, user_id: str, images_data: List[Dict]) -> List[Dict]:
        ...
        for i, img_data in enumerate(images_data):
            ...
            if ext not in supported_formats:
                raise ValueError(
                    f"Изображение {filename} не является поддерживаемого формата (поддерживаются: JPEG, HEIC, GIF, BMP, PNG)"
                )
            ...
            if file_size > max_size:
                raise ValueError(
                    f"Изображение {filename} слишком большое: {file_size} байт. Максимум: {max_size} байт (24 МБ)"
                )

            # Согласно документации: Request Body schema: multipart/form-data с полем uploadfile[]
            files = {"uploadfile[]": (filename, io.BytesIO(file_data), mime_type)}
        ...
        if media_data:
            for media_item in media_data:
                try:
                    ...
                    upload_result = self.upload_media_from_bytes(user_id, file_data, filename, file_type, mime_type)
                    attachment_id = upload_result.get("attachment_id") or upload_result.get("id")
                    if attachment_id:
                        attachments.append({"id": attachment_id})
                except Exception as e:
    ...
    def send_image_message(
        ...
        if image_path:
            ...
            upload_results = self.upload_images_from_bytes(user_id, [{"data": image_data, "filename": filename}])
