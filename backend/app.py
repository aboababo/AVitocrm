...
from database import get_connection, init_database, is_database_initialized, safe_init_database
...
try:
    ...
    app.logger.warning("[INIT] flask-limiter не установлен, rate limiting отключен")
...
if os.environ.get("SECRET_KEY"):
    ...
    # Используем из конфига или генерируем новый
    ...
    if app.secret_key == "dev-secret-key-change-in-production":
        ...
        try:
            ...
            app.logger.warning("[INIT] [WARNING] ВНИМАНИЕ: При перезапуске сервера все сессии станут невалидными!")
...
# Backwards-compatible wrapper so existing code using `get_db_connection()`
# continues to work but uses the new context-managed `get_connection()`.
from flask import g


def get_db_connection():
    """Return a per-request sqlite connection managed by `get_connection()`.

    This keeps existing code working while ensuring the connection is
    closed at the end of the request.
    """
    if getattr(g, "_db_conn_ctx", None) is None:
        g._db_conn_ctx = get_connection()
        g._db_conn = g._db_conn_ctx.__enter__()
    return g._db_conn


@app.teardown_appcontext
def _close_db_connection(exception=None):
    ctx = getattr(g, "_db_conn_ctx", None)
    if ctx is not None:
        try:
...
if LIMITER_AVAILABLE:
    ...
    app.logger.info("[INIT] Rate limiting включен")
else:
    limiter = None
...
def _attempt_init():
    """Try to initialize DB using safe_init_database, fall back to init_database.
    ...
    try:
        ...
        app.logger.warning(f"[INIT] safe_init_database() failed: {e}")
    # If still not initialized, try regular init
    if not is_database_initialized():
        try:
            init_database()
        except Exception as e:
            app.logger.error(f"[INIT] init_database() failed: {e}", exc_info=True)
    return is_database_initialized()

try:
    if _attempt_init():
        # Touch a connection to ensure everything works
        try:
        ...
        app.logger.info("[INIT] База данных успешно инициализирована")
    else:
        app.logger.error("[INIT] Не удалось инициализировать базу данных")
except Exception as e:
    app.logger.error(f"[INIT] Критическая ошибка при инициализации БД: {e}", exc_info=True)
# Регистрируем health/readiness/metrics
register_health_routes(app)

# Регистрируем blueprints для модульных маршрутов
try:
    from routes.analytics_routes import analytics_bp
    from routes.auth_routes import auth_bp
    from routes.deliveries_routes import deliveries_bp
    from routes.managers_routes import managers_bp
    from routes.messages_routes import messages_bp
    from routes.page_routes import pages_bp
    from routes.quick_replies_routes import quick_replies_bp
    from routes.settings_routes import settings_bp
    from routes.shops_routes import shops_bp
    ...
    app.logger.info("[INIT] Все blueprints успешно зарегистрированы")
except Exception as e:
    app.logger.error(f"[INIT] Ошибка регистрации blueprints: {e}", exc_info=True)

# Регистрируем blueprints для API
# Используем модульную структуру chats API
try:
    from api.chats.chats_batch import batch_bp
    from api.chats.chats_endpoints import chats_bp
    from api.chats.chats_listing import listing_bp

    app.register_blueprint(chats_bp)
    app.register_blueprint(batch_bp)
    app.register_blueprint(listing_bp)

    app.logger.info("[INIT] Blueprints chats API успешно зарегистрированы")
except Exception as e:
    app.logger.warning(f"[INIT] Не удалось импортировать blueprints chats API: {e}")
    # Fallback на старый импорт
    try:
        from api.chats_api import chats_bp

        app.logger.info("[INIT] Используется старый chats_api blueprint")
    except Exception as e2:
        app.logger.warning(f"[INIT] Не удалось импортировать старый chats_api: {e2}")
...
def inject_user():
    ...
    try:
        ...
        # Если нет контекста запроса — возвращаем пустые значения
        try:
        ...
        # Безопасно пытаемся получить user_id из session
        try:
        ...
        # Попытка загрузить индивидуальные настройки из БД
        try:
            with get_connection() as conn:
                try:
                    ...
                    if user_setting and user_setting.get("tab_visibility"):
                        import json
                    ...
                    app.logger.warning(
                        f"[INJECT_USER] Ошибка загрузки настроек видимости вкладок: {e}"
                    )
                    tab_visibility = {
                        "dashboard": True,
                        "chats": True,
    ...
    def decorated_function(*args, **kwargs):
        ...
        if "user_id" not in session:
            ...
            if request.path.startswith("/api/"):
                ...
                app.logger.warning(
                    f"[REQUIRE_AUTH] Cookie present: {cookie_header != 'None'}, Session keys: {session_keys}"
                )
                if cookie_header != "None" and len(session_keys) == 0:
                    app.logger.warning(
                        "[REQUIRE_AUTH] [WARNING] Cookie отправлен, но сессия не расшифрована! Вероятно, SECRET_KEY изменился."
                    )
                return (
                    jsonify(
                        {
...
def handle_not_found(error):
    ...
    if request.is_json or request.path.startswith("/api/"):
        return jsonify({"error": "Ресурс не найден", "error_code": "NOT_FOUND"}), 404
...
def handle_internal_error(error):
    ...
    if request.is_json or request.path.startswith("/api/"):
        return (
            jsonify({"error": "Внутренняя ошибка сервера", "error_code": "INTERNAL_ERROR"}),
            500,
        )
    return render_template("error.html", error="Внутренняя ошибка сервера"), 500
...
if LIMITER_AVAILABLE:
    ...
    def handle_rate_limit(error):
        """Обработчик для rate limit ошибок"""
        return (
            jsonify(
                {
...
def get_system_stats():
    ...
    # Используем оптимизированную функцию для получения всех статистик одним запросом
    from utils.db_optimizer import get_stats_batch

    with get_connection() as conn:
        stats_data = get_stats_batch(conn)

    total_chats = stats_data["total_chats"]
    active_chats = stats_data["active_chats"]
    urgent_chats = stats_data["urgent_chats"]
    unread_chats = stats_data["unread_chats"]
    pool_chats = stats_data["pool_chats"]
    avg_response_time = stats_data["avg_response_time"]
    total_users = stats_data["total_users"]
    total_managers = stats_data["total_managers"]
    total_shops = stats_data["total_shops"]
    shops_with_keys = stats_data["shops_with_keys"]
...
@handle_errors
def login():
    # Применяем rate limiting для POST запросов
    if request.method == "POST" and LIMITER_AVAILABLE and limiter:
        ...
        try:
            ...
            # Если превышен лимит, flask-limiter сам вернет 429
            pass
        ...
        with get_connection() as conn:
            ...
            if user_info:
                log_activity(
                    ...
                    f'Обновлена видимость вкладок для пользователя: {user_info["username"]} ({user_info["role"]})',
                    "user",
                    user_id,
                )
...
def get_shops():
    ...
    from utils.api_helpers import get_paginated_response

    # removed unnecessary direct DB open; pagination helper manages DB access
    user_role = session.get("user_role")
    ...
    if user_role in ["admin", "super_admin"]:
        ...
        query = """
            ...
            ORDER BY created_at DESC
        """
        result = get_paginated_response(query)
    else:
        ...
        """
        result = get_paginated_response(query, (session["user_id"],))

    # Скрываем ключи только для менеджеров
    for shop in result["items"]:
        if user_role not in ["admin", "super_admin"]:
            shop.pop("client_id", None)
            shop.pop("client_secret", None)
            shop.pop("user_id", None)

    app.logger.info(f'[GET SHOPS] Загружено магазинов для {user_role}: {result["total"]}')
    ...
    try:
        ...
        for idx, shop in enumerate(shops):
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        if isinstance(response, dict) and "meta" in response:
                            ...
                            app.logger.info(
                                f"[SYNC] Метаданные: total={total}, has_more={has_more}, offset={offset}, limit={limit}"
                            )
                            if total > 0:
                                app.logger.info(
                                    f"[SYNC] Всего чатов в Avito: {total}, будет обработано страниц: {(total + limit - 1) // limit}"
                                )
                            elif has_more:
                                # Если has_more=True, но total=0, значит нужно продолжать пагинацию
                                app.logger.info(
                                    "[SYNC] has_more=True, продолжаем пагинацию (total может быть не указан)"
                                )

                        if not chats_data or chats_count == 0:
                            app.logger.info("[SYNC] Нет чатов для обработки, завершаем пагинацию")
                            break
                            ...
                            try:
                                ...
                                if not avito_chat_id:
                                    if idx < 3:  # Логируем только первые 3 для отладки
                                        app.logger.warning(
                                            f"[SYNC] Чат {idx}: нет ID, пропускаем. Данные: {list(chat_data.keys())}"
                                        )
                                    continue
                                ...
                                if not existing:
                                    ...
                                    if existing_by_chat_id:
                                        ...
                                        app.logger.warning(
                                            f"[SYNC] Чат {avito_chat_id_str} найден с другим shop_id: БД={existing_by_chat_id.get('shop_id')}, текущий={shop['id']}"
                                        )
                                ...
                                if should_log:
                                    app.logger.info(
                                        f"[SYNC] Чат {idx} (chat_id={avito_chat_id_str}): проверяем наличие context/item/listing/ad в chat_data, ключи: {list(chat_data.keys())}"
                                    )
                                    if "context" in chat_data:
                                        app.logger.info(
                                            f"[SYNC] Чат {idx}: context тип={type(chat_data.get('context'))}, ключи: {list(chat_data.get('context', {}).keys()) if isinstance(chat_data.get('context'), dict) else 'не dict'}"
                                        )
                                        if isinstance(chat_data.get("context"), dict):
                                            app.logger.info(
                                                f"[SYNC] Чат {idx}: context содержимое: {str(chat_data.get('context'))[:500]}"
                                            )
                                    if item_data:
                                        app.logger.info(
                                            f"[SYNC] Чат {idx}: item_data тип={type(item_data)}, значение={str(item_data)[:500]}"
                                        )
                                    else:
                                        app.logger.warning(
                                            f"[SYNC] Чат {idx}: item_data отсутствует! Все ключи chat_data: {list(chat_data.keys())}"
                                        )
                                ...
                                if isinstance(item_data, dict) and item_data:
                                    ...
                                    app.logger.info(
                                        f"[SYNC] Чат {idx}: Сохраняем listing_data из context.value (ключи: {list(item_data.keys())[:10]})"
                                    )
                                ...
                                if (
                                    not product_url
                                    and shop.get("client_id")
                                    and shop.get("client_secret")
                                    and shop.get("user_id")
                                ):
                                    try:
                                        from avito_api import AvitoAPI
                                        ...
                                        if isinstance(chat_details, dict):
                                            ...
                                            if isinstance(detail_context, dict):
                                                # Приоритет: context.value (API v3), затем context.item (старая версия)
                                                detail_item = (
                                                    detail_context.get("value")
                                                    or detail_context.get("item")
                                                    or detail_context.get("listing")
                                                    or detail_context.get("ad", {})
                                                )
                                                if isinstance(detail_item, dict) and detail_item:
                                                    ...
                                                    if not listing_data_json:
                                                        ...
                                                        listing_data_json = json.dumps(
                                                            detail_item,
                                                            ensure_ascii=False,
                                                        )
                                                        app.logger.info(
                                                            f"[SYNC] Чат {idx}: Сохраняем listing_data из get_chat_by_id context.value (ключи: {list(detail_item.keys())[:10]})"
                                                        )

                                                    detail_item_id = detail_item.get("id")
                                                    detail_url = (
                                                        detail_item.get("url")
                                                        or detail_item.get("link")
                                                        or detail_item.get("href")
                                                        or detail_item.get("value")
                                                        or detail_item.get("uri")
                                                    )
                                                    if detail_url:
                                                        ...
                                                        if product_url.startswith("/"):
                                                            ...
                                                            product_url = f"https://www.avito.ru{product_url}"
                                                        app.logger.info(
                                                            f"[SYNC] [OK] Чат {idx}: product_url найден через get_chat_by_id context.value (url): {product_url}"
                                                        )
                                                    elif detail_item_id:
                                                        item_id_str = str(detail_item_id)
                                                        shop_url_part = (
                                                            shop.get("shop_url", "").split("/")[-1]
                                                        ...
                                                        if shop_url_part:
                                                            ...
                                                            product_url = f"https://www.avito.ru/items/{item_id_str}"
                                                        app.logger.info(
                                                            f"[SYNC] [OK] Чат {idx}: product_url найден через get_chat_by_id context.value (id): {product_url}"
                                                        )

                                            # Если не нашли в context, проверяем прямые поля
                                            if not product_url:
                                                product_url = (
                                                    chat_details.get("item_url")
                                                    or chat_details.get("listing_url")
                                                    or chat_details.get("ad_url")
                                                    or chat_details.get("product_url")
                                                )
                                                if product_url:
                                                    app.logger.info(
                                                        f"[SYNC] [OK] Чат {idx}: product_url найден через get_chat_by_id (прямые поля): {product_url}"
                                                    )

                                            if not product_url:
                                                app.logger.warning(
                                                    f"[SYNC] [WARNING] Чат {idx}: product_url не найден даже через get_chat_by_id. Ключи chat_details: {list(chat_details.keys())}"
                                                )
                                                if "context" in chat_details:
                                                    app.logger.warning(
                                                        f"[SYNC] [WARNING] Чат {idx}: context = {str(chat_details.get('context'))[:500]}"
                                                    )
                                    except Exception as api_error:
                                        app.logger.warning(
                                            f"[SYNC] Чат {idx}: ошибка при попытке получить product_url через get_chat_by_id: {api_error}"
                                        )

                                if product_url:
                                    app.logger.info(
                                        f"[SYNC] Чат {idx} (chat_id={avito_chat_id_str}): найден product_url={product_url}"
                                    )
                                else:
                                    app.logger.warning(
                                        f"[SYNC] Чат {idx} (chat_id={avito_chat_id_str}): product_url НЕ найден. Возможно, чат не связан с объявлением или объявление было удалено."
                                    )
                                    # Логируем структуру chat_data для диагностики
                                    if idx < 5:  # Логируем первые 5 чатов без product_url
                                        app.logger.warning(
                                            f"[SYNC] Чат {idx}: структура chat_data - ключи: {list(chat_data.keys())}"
                                        )
                                        if "context" in chat_data:
                                            app.logger.warning(
                                                f"[SYNC] Чат {idx}: context = {str(chat_data.get('context'))[:300]}"
                                            )
                                ...
                                if existing:
                                    ...
                                    try:
                                        ...
                                        if product_url:
                                            app.logger.info(
                                                f"[SYNC] Обновление чата {existing_id} с product_url={product_url}"
                                            )
                                        ...
                                        if product_url:
                                            ...
                                            if verify_chat:
                                                ...
                                                app.logger.info(
                                                    f"[SYNC] Проверка сохранения для чата {existing_id}: product_url в БД = {saved_url}"
                                                )
                                                if saved_url != product_url:
                                                    app.logger.error(
                                                        f"[SYNC] ОШИБКА: product_url не совпадает! Ожидалось: {product_url}, Сохранено: {saved_url}"
                                                    )
                                            else:
                                                app.logger.error(
                                                    f"[SYNC] ОШИБКА: не удалось проверить сохранение product_url для чата {existing_id}"
                                                )
                                    except Exception as update_err:
                                        app.logger.error(
                                            f"[SYNC] Ошибка обновления чата {avito_chat_id_str}: {update_err}",
                                            exc_info=True,
                                        )
                                    ...
                                    try:
                                        ...
                                        total_synced += 1
                                        app.logger.info(
                                            f"[SYNC] [OK] Создан новый чат: БД_id={new_chat_id}, shop_id={shop['id']}, chat_id={avito_chat_id_str}, client_name={client_name}, product_url={product_url}"
                                        )
                                        ...
                                        if product_url:
                                            ...
                                            if verify_chat:
                                                ...
                                                verify_chat = dict(verify_chat)
                                                app.logger.info(
                                                    f"[SYNC] Проверка сохранения для нового чата {new_chat_id}: product_url в БД = {verify_chat.get('product_url')}"
                                                )
                                    except Exception as insert_err:
                                        app.logger.error(
                                            f"[SYNC] [ERROR] Ошибка создания чата {avito_chat_id_str}: {insert_err}",
                                            exc_info=True,
                                        )
                                        app.logger.error(
                                            f"[SYNC] Данные чата: shop_id={shop['id']}, client_name={client_name}, customer_id={customer_id}"
                                        )
                                        if "chats_errors" in locals():
                                            chats_errors += 1
                                        else:
                                ...
                                if chats_processed % 10 == 0:
                                    app.logger.info(
                                        f"[SYNC] Обработано {chats_processed}/{chats_count} чатов (создано: {chats_created}, обновлено: {chats_updated})"
                                    )
                                ...
                                try:
                                    ...
                                    chat_id_str = "unknown"
                                app.logger.error(
                                    f"[SYNC] Ошибка обработки чата {chat_id_str}: {e}",
                                    exc_info=True,
                                )
                        ...
                        app.logger.info(
                            f"[SYNC] Страница завершена: обработано={chats_processed}, создано={chats_created}, обновлено={chats_updated}, ошибок={chats_errors}"
                        )
                        ...
                        actual_count = conn.execute(
                            "SELECT COUNT(*) as cnt FROM avito_chats WHERE shop_id = ?",
                            (shop["id"],),
                        ).fetchone()
                        if actual_count:
                            actual_count = dict(actual_count)
                            app.logger.info(
                                f"[SYNC] Изменения сохранены в БД (всего синхронизировано: {total_synced}, реально чатов в БД для shop_id={shop['id']}: {actual_count.get('cnt', 0)})"
                            )
                        else:
                            app.logger.info(
                                f"[SYNC] Изменения сохранены в БД (всего синхронизировано: {total_synced}, реально чатов в БД для shop_id={shop['id']}: 0)"
                            )
                        ...
                        if chats_count < limit and not has_more:
                            app.logger.info(
                                f"[SYNC] Получено меньше лимита ({chats_count} < {limit}) и has_more=False, завершаем пагинацию"
                            )
                            break
                        elif chats_count == 0 and not has_more:
                            app.logger.info("[SYNC] Нет чатов и has_more=False, завершаем пагинацию")
                            break
                        elif has_more:
                        ...
                        if offset >= 1000:
                            app.logger.warning(
                                f"[SYNC] Достигнут лимит offset={offset}, завершаем пагинацию для защиты от бесконечного цикла"
                            )
                            break
                        ...
                        if "403" in error_str or "Forbidden" in error_str:
                            app.logger.warning(
                                f"[SYNC] [WARNING] Магазин {shop['id']} ({shop['name']}): 403 Forbidden - ключи не работают или нет доступа"
                            )
                            app.logger.warning(
                                f"[SYNC] Пропускаем магазин {shop['id']} и продолжаем с другими магазинами"
                            )
                ...
                conn.commit()
                app.logger.info(
                    f"[SYNC] [OK] Магазин {shop['id']} ({shop['name']}) синхронизирован: {total_synced} чатов"
                )
                ...
                if "403" in error_str or "Forbidden" in error_str:
                    ...
                    app.logger.warning(f"[SYNC] [WARNING] {error_msg}")
                    ...
                    app.logger.error(f"[SYNC] [ERROR] {error_msg}", exc_info=True)
                errors.append(error_msg)
                app.logger.info(
                    f"[SYNC] ========== Ошибка синхронизации магазина {shop['id']}: {shop['name']} =========="
                )
                continue

        app.logger.info("[SYNC] ========== Синхронизация завершена ==========")
        ...
        if errors:
            ...
            app.logger.info("[SYNC] Ошибок нет")

        # Автоматически обновляем таймеры для всех чатов после синхронизации
        try:
            app.logger.info("[SYNC] 🔄 Начинаем обновление таймеров для всех чатов...")
            ...
            app.logger.info(
                f"[SYNC] [OK] Таймеры обновлены: обновлено={timer_result['updated']}, ошибок={timer_result['errors']}"
            )
        except Exception as timer_update_error:
            app.logger.warning(f"[SYNC] [WARNING] Ошибка обновления таймеров: {timer_update_error}")

        # Автоматически завершаем старые чаты (старше 1 суток)
        try:
            app.logger.info("[SYNC] 🔄 Начинаем автозавершение старых чатов...")
            from services.messenger_service import MessengerService

            service = MessengerService(conn, None)
            complete_result = service.auto_complete_old_chats(days=1)
            app.logger.info(
                f"[SYNC] [OK] Автозавершение завершено: завершено={complete_result['completed']}, ошибок={complete_result['errors']}"
            )
        except Exception as auto_complete_error:
            app.logger.warning(f"[SYNC] [WARNING] Ошибка автозавершения чатов: {auto_complete_error}")
...
def get_chats():
    ...
    try:
        ...
        return (
            jsonify({"error": "Internal server error", "message": str(flask_exception)}),
            500,
        )
...
def _get_chats_impl():
    ...
    # Используем оптимизированную функцию для построения запроса
    chats = []  # Инициализируем пустым списком на случай ошибки
    try:
        from utils.db_optimizer import get_chats_optimized

        # Используем оптимизированную функцию
        chats = get_chats_optimized(
            conn,
            show_pool=show_pool,
            include_completed=include_completed,
            updated_since=updated_since_timestamp,
            has_name_columns=has_name_columns,
        )
    except Exception as fallback_error:
        app.logger.warning(f"[API/CHATS] Ошибка оптимизированного запроса, используем fallback: {fallback_error}")
        # Fallback на старый код если оптимизация не работает
        try:
            if show_pool:
                ...
                if include_completed:
                    ...
                    if has_name_columns:
                        ...
                        ORDER BY 
                            ...
                            c.updated_at DESC
                    """,
                            ((updated_since_timestamp,) if updated_since_timestamp else ()),
                        ).fetchall()
                    else:
                        chats = conn.execute(
                            """
        ...
        except Exception as fallback_inner_error:
            app.logger.error(
                f"[API/CHATS] Ошибка в fallback запросе: {fallback_inner_error}",
                exc_info=True,
            )
            chats = []
        ...
        app.logger.warning("[API/CHATS] Используем fallback запрос на username")
        try:
            if show_pool:
                if include_completed:
                    chats = conn.execute(
            ...
            app.logger.error(
                f"[API/CHATS] Критическая ошибка даже в fallback запросе: {fallback_error}",
                exc_info=True,
            )
            return (
                jsonify({"error": "Internal server error", "message": str(fallback_error)}),
                500,
            )
    ...
    if not chats or len(chats) == 0:
        try:
            ...
            app.logger.warning(
                f"[API/CHATS] [WARNING] SQL запрос вернул 0 результатов, но в таблице avito_chats всего {total_count} чатов"
            )

            # Проверяем, есть ли чаты с назначенными менеджерами
            if show_pool:
                pool_chats_count = conn.execute(
                    "SELECT COUNT(*) as count FROM avito_chats WHERE assigned_manager_id IS NULL"
                ).fetchone()
                pool_count = pool_chats_count[0] if pool_chats_count else 0
                app.logger.warning(
                    f"[API/CHATS] [WARNING] В пуле (assigned_manager_id IS NULL) должно быть {pool_count} чатов"
                )
        except Exception as count_error:
            app.logger.error(
                f"[API/CHATS] Ошибка при проверке количества чатов: {count_error}",
                exc_info=True,
            )
    ...
    try:
        ...
        if chat_ids:
            try:
                    ...
                    try:
                        ...
                        if chat_id and last_unanswered_time_str:
                            ...
                            try:
                                ...
                                app.logger.warning(
                                    f"[API/CHATS] Ошибка парсинга timestamp для чата {chat_id}: {parse_error}, timestamp: {last_unanswered_time_str}"
                                )
                ...
                app.logger.warning(
                    f"[API/CHATS] Ошибка вычисления response_timer: {timer_calc_error}",
                    exc_info=True,
                )
        ...
        for chat_dict in chats_list:
            try:
                ...
                app.logger.error(
                    f"[API/CHATS] Ошибка обработки чата {chat_dict.get('id', 'unknown')}: {chat_process_error}",
                    exc_info=True,
                )
        ...
        app.logger.error(
            f"[API/CHATS] Критическая ошибка при обработке результатов: {process_error}",
            exc_info=True,
        )
        return (
            jsonify({"error": "Internal server error", "message": str(process_error)}),
            500,
        )
...
def update_all_timers():
    ...
    try:
        ...
        # Используем контекстный менеджер для безопасного закрытия соединения
        with get_connection() as conn:
...
def auto_complete_chats():
    ...
    try:
        ...
        # Контекстный менеджер обеспечивает закрытие соединения
        with get_connection() as conn:
...
def sync_chats():
    ...
    cookie_header = request.headers.get("Cookie", "None")
    app.logger.info(
        f"[API/CHATS/SYNC] Запрос получен. Session keys: {list(session.keys())}, user_id: {session.get('user_id')}"
    )
    app.logger.info(
        f"[API/CHATS/SYNC] Cookie present: {cookie_header != 'None'}, Cookie length: {len(cookie_header) if cookie_header != 'None' else 0}"
    )
    ...
    try:
        ...
        return (
            jsonify({"success": False, "error": f"Ошибка синхронизации: {str(e)}"}),
            500,
        )
...
def create_shop():
    ...
    user_role = session.get("user_role")
    if user_role not in ["admin", "super_admin"]:
        raise PermissionDeniedError("Требуется роль admin или super_admin")

    # Валидация данных
    from utils.api_helpers import validate_request_data
    from utils.schemas import ShopCreateSchema

    try:
        data = validate_request_data(ShopCreateSchema)
    except ValidationError as e:
        raise e

    name = data["name"]
    avito_user_id = data.get("avito_user_id", "")
    ...
    if shop_url and not shop_url.startswith(("http://", "https://")):
        raise ValidationError("URL магазина должен начинаться с http:// или https://")

    with get_connection() as conn:
        try:
            cursor = conn.execute(
                """
                INSERT INTO avito_shops (name, shop_url, api_key, user_id, client_id, client_secret, is_active)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    name,
...
def update_shop_credentials(shop_id):
    ...
    with get_connection() as conn:
        ...
        app.logger.debug(
            f"[SHOPS] Текущие значения: client_id={'*' if current['client_id'] else 'None'}, user_id={current['user_id']}"
        )
        ...
        app.logger.info(
            f'[UPDATE CREDENTIALS] Ожидалось: client_id={expected_client_id[:10] if expected_client_id else "None"}..., user_id={expected_user_id}'
        )
        app.logger.info(
            f'[UPDATE CREDENTIALS] Получено: client_id={verify_client_id[:10] if verify_client_id else "None"}..., user_id={verify_user_id}'
        )

        if verify_client_id != expected_client_id or verify_user_id != expected_user_id:
            app.logger.error(
                f'[UPDATE CREDENTIALS] Данные не сохранились! Ожидалось: client_id={expected_client_id[:10] if expected_client_id else "None"}..., user_id={expected_user_id}, получено: client_id={verify_client_id[:10] if verify_client_id else "None"}..., user_id={verify_user_id}'
            )
            # Соединение глобальное, не закрываем
            return (
                jsonify({"error": "Данные не были сохранены. Попробуйте еще раз."}),
                500,
            )
        ...
        try:
            ...
            if sync_result.get("success"):
                ...
                app.logger.warning(
                    f'[UPDATE CREDENTIALS] Ошибка синхронизации чатов: {sync_result.get("error", "Unknown error")}'
                )
    ...
    try:
        for update in updates:
            ...
            if session.get("user_role") != "admin":
                ...
                check = conn.execute("SELECT manager_id FROM deliveries WHERE id = ?", (delivery_id,)).fetchone()
        ...
        log_activity(
            ...
            f"Массовое обновление {updated_count} доставок",
            "delivery",
            None,
        )
...
def get_chat_messages(chat_id):
    ...
    app.logger.info(
        f"[API/MESSAGES] Cookie present: {cookie_header != 'None'}, Cookie length: {len(cookie_header) if cookie_header != 'None' else 0}"
    )
    app.logger.info(
        f"[API/MESSAGES] Session keys: {session_keys}, has_user_id: {has_user_id}, user_id: {user_id_value}"
    )
    ...
    if not has_user_id:
        ...
        if len(session_keys) == 0:
            app.logger.warning(
                "[API/MESSAGES] [WARNING] Сессия полностью пустая! Cookie был отправлен, но сессия не расшифрована."
            )
            app.logger.warning(
                "[API/MESSAGES] Возможные причины: SECRET_KEY изменился, сессия истекла, или проблема с декодированием"
            )
        else:
            app.logger.warning(f"[API/MESSAGES] [WARNING] В сессии есть данные ({session_keys}), но нет user_id!")
    ...
    with get_connection() as conn:
        ...
        app.logger.info(
            f"[API/MESSAGES] client_id={bool(chat_dict.get('client_id'))}, client_secret={bool(chat_dict.get('client_secret'))}, user_id={chat_dict.get('user_id')}"
        )
        app.logger.info(
            f"[API/MESSAGES] Условие синхронизации: sync={sync}, client_id={bool(chat_dict.get('client_id'))}, client_secret={bool(chat_dict.get('client_secret'))}, user_id={bool(chat_dict.get('user_id'))}"
        )
        ...
        if should_sync:
            if time_since_last_sync < SYNC_COOLDOWN:
                app.logger.info(
                    f"[API/MESSAGES] ⏸️ Синхронизация пропущена: прошло только {time_since_last_sync:.1f} сек (минимум {SYNC_COOLDOWN} сек)"
                )
                should_sync = False
            else:
                # Обновляем время последней синхронизации
                setattr(get_chat_messages, last_sync_key, current_time)
                app.logger.info(f"[API/MESSAGES] [OK] Синхронизация разрешена: прошло {time_since_last_sync:.1f} сек")

        if should_sync:
            app.logger.info("[API/MESSAGES] [OK] Условие синхронизации выполнено, начинаем синхронизацию...")
            try:
                avito_chat_id = chat_dict.get("chat_id")
                if not avito_chat_id:
                    app.logger.error(
                        f"[API/MESSAGES] [ERROR] avito_chat_id не найден в chat_dict! Доступные ключи: {list(chat_dict.keys())}"
                    )
                    raise ValueError(f"avito_chat_id не найден для чата {chat_id}")
                app.logger.info(
                    f"[API/MESSAGES] Начинаем синхронизацию сообщений для чата {chat_id}, user_id={chat_dict.get('user_id')}, avito_chat_id={avito_chat_id}"
                )
                from avito_api import AvitoAPI
                from services.messenger_service import MessengerService
                ...
                app.logger.info(
                    f"[API/MESSAGES] Синхронизация завершена: {new_messages_count} новых сообщений для чата {chat_id}"
                )
            except Exception as sync_error:
                app.logger.error(
                    f"[API/MESSAGES] Ошибка синхронизации сообщений для чата {chat_id}: {sync_error}",
                    exc_info=True,
                )
                import traceback
            ...
            if sync:
                app.logger.warning(
                    f"[API/MESSAGES] [WARNING] Синхронизация запрошена, но не выполнена: client_id={bool(chat_dict.get('client_id'))}, client_secret={bool(chat_dict.get('client_secret'))}, user_id={bool(chat_dict.get('user_id'))}"
                )
            else:
                app.logger.info("[API/MESSAGES] Синхронизация не запрошена (sync=false)")
    except Exception as messages_error:
        app.logger.error(
            f"[API/MESSAGES] Критическая ошибка при получении данных чата {chat_id}: {messages_error}",
            exc_info=True,
        )
        import traceback
    ...
    try:
        ...
        app.logger.info(
            f"[API/MESSAGES] Возвращаем {len(messages)} сообщений из {total_count} всего для чата {chat_id}"
        )
        if len(messages) > 0:
            first_msg = dict(messages[0])
            app.logger.info(
                f"[API/MESSAGES] Первое сообщение: id={first_msg.get('id')}, text={first_msg.get('message_text', '')[:50]}..."
            )
        else:
            app.logger.warning(f"[API/MESSAGES] [WARNING] Нет сообщений для чата {chat_id} (total={total_count})")
            # Проверяем, есть ли last_message в таблице чатов
            chat_info_check = conn.execute(
                "SELECT last_message, chat_id FROM avito_chats WHERE id = ?", (chat_id,)
            ).fetchone()
            if chat_info_check and chat_info_check["last_message"]:
                app.logger.warning(
                    f"[API/MESSAGES] [WARNING] В avito_chats.last_message есть данные, но в avito_messages нет! chat_id={chat_info_check['chat_id']}"
                )
        ...
        app.logger.error(
            f"[API/MESSAGES] Ошибка при запросе сообщений для чата {chat_id}: {query_error}",
            exc_info=True,
        )
        import traceback
    ...
    try:
        ...
        if chat_dict.get("client_id") and chat_dict.get("client_secret") and chat_dict.get("user_id"):
            try:
                ...
                chat_details = api.get_chat_by_id(user_id=chat_dict["user_id"], chat_id=chat_dict["chat_id"])
                    ...
                    if isinstance(detail_context, dict):
                        ...
                        if isinstance(detail_item, dict):
                            ...
                            if detail_url:
                                ...
                                item_id_str = str(detail_item_id)
                                shop_url_part = (
                                    chat_dict.get("shop_url", "").split("/")[-1] if chat_dict.get("shop_url") else ""
                                )
                                if shop_url_part:
                                    product_url = f"https://www.avito.ru/{shop_url_part}/items/{item_id_str}"
                                else:
                                ...
                                app.logger.info(
                                    f"[EXTRACT PRODUCT URL] Найден через API context.value.id: {product_url}"
                                )
                            ...
                            if isinstance(d, dict):
                                for key, value in d.items():
                                    if isinstance(value, str) and (
                                        "avito.ru" in value.lower() or "/items/" in value.lower()
                                    ):
            ...
            if not product_url:
                ...
                for item_id in potential_item_ids[:5]:
                    ...
                    url_in_messages = any(
                        test_url.lower() in (msg_row["message_text"] or "").lower() for msg_row in all_messages[:20]
                    )
                    ...
                    if url_in_messages:
                        ...
                        source = "messages_id"
                        app.logger.info(
                            f"[EXTRACT PRODUCT URL] Найден item_id в сообщениях: {item_id}, URL: {product_url}"
                        )
                        break
        ...
        if product_url:
            ...
            app.logger.info(
                f"[EXTRACT PRODUCT URL] Для чата {chat_id} найден product_url: {product_url} (источник: {source})"
            )
            # Соединение глобальное, не закрываем
            return (
                jsonify(
...
def get_chat_listing(chat_id):
    ...
    app.logger.info("[CHAT LISTING] ========== НАЧАЛО ЗАГРУЗКИ ОБЪЯВЛЕНИЯ ДЛЯ ЧАТА ==========")
    ...
    try:
        ...
        try:
            result = ChatListingService.get_chat_listing(chat_id=chat_id, user_id=user_id, user_role=user_role)
        except Exception as service_error:
            app.logger.error(
                f"[CHAT LISTING] Ошибка в сервисе get_chat_listing: {service_error}",
                exc_info=True,
            )
            import traceback
        ...
        app.logger.info("[CHAT LISTING] ========== ЗАГРУЗКА ОБЪЯВЛЕНИЯ ЗАВЕРШЕНА УСПЕШНО ==========")
        return jsonify(result)
        ...
        app.logger.error(
            f"[CHAT LISTING] ОШИБКА получения информации об объявлении: {str(e)}",
            exc_info=True,
        )
        import traceback
...
def send_message(chat_id):
    ...
    try:
        ...
        app.logger.info("[SEND MESSAGE] Проверка условий для отправки:")
        ...
        app.logger.info(
            f"[SEND MESSAGE]   client_secret: {bool(chat.get('client_secret'))} ({'***' if chat.get('client_secret') else None})"
        )
        ...
        if chat.get("client_id") and chat.get("client_secret") and chat.get("shop_user_id") and chat.get("chat_id"):
            app.logger.info("[SEND MESSAGE] [OK] Все условия выполнены, начинаем отправку через Avito API")
            app.logger.info(
                f"[SEND MESSAGE] Начало отправки: chat_id={chat_id}, avito_chat_id={chat.get('chat_id')}, user_id={chat.get('shop_user_id')}, message_len={len(message)}"
            )
            try:
                from avito_api import AvitoAPI

                app.logger.info("[SEND MESSAGE] Создание экземпляра AvitoAPI...")
                api = AvitoAPI(
                    client_id=chat.get("client_id"),
                    client_secret=chat.get("client_secret"),
                )
                app.logger.info("[SEND MESSAGE] [OK] AvitoAPI создан, вызываем api.send_message...")
                app.logger.info(
                    f"[SEND MESSAGE] Параметры: user_id={chat.get('shop_user_id')}, chat_id={chat.get('chat_id')}, message_len={len(message)}"
                )
                ...
                app.logger.info(f"[SEND MESSAGE] [OK] Успешно отправлено для чата {chat_id}")
                ...
                if "405" in str(e) or "HTTP 405" in str(e):
                    ...
                    app.logger.error("=" * 80)
                    app.logger.error("[SEND MESSAGE] ОШИБКА 405 ПРИ ОТПРАВКЕ СООБЩЕНИЯ")
                    ...
                    app.logger.error("=" * 80)
                app.logger.error(f"[SEND MESSAGE] [ERROR] Ошибка отправки для чата {chat_id}: {e}")
                app.logger.error(f"[SEND MESSAGE] Тип ошибки: {type(e).__name__}")
                import traceback
            ...
            app.logger.warning(
                f"[SEND MESSAGE] [WARNING] Пропуск отправки через Avito API: отсутствуют {', '.join(missing)}"
            )
            app.logger.warning("[SEND MESSAGE] Сообщение будет сохранено только в БД")
        ...
        if avito_message_sent:
            ...
            if "405" in str(avito_error) or "HTTP 405" in str(avito_error):
                response_data["warning"] = (
                    f"Сообщение сохранено в БД, но не отправлено в Avito: Ошибка API запроса: {avito_error}"
                )
...
def upload_media():
    ...
    try:
        ...
        if not attachment_id:
            return (
                jsonify({"error": "Failed to get attachment_id from upload response"}),
                500,
            )
...
def avito_webhook():
    ...
    try:
        ...
        if event_type in ["message", "new_message"]:
            ...
            app.logger.info(
                f"[WEBHOOK] Обработка сообщения: chat_id={avito_chat_id}, user_id={user_id}, message_id={message_id}"
            )
            ...
            if shop:
                ...
                if not chat:
                    ...
                    app.logger.info(
                        f"[WEBHOOK] Чат {avito_chat_id} не найден в БД, синхронизируем все чаты магазина {shop_dict['id']}"
                    )
                    try:
                        sync_result = sync_chats_from_avito(shop_id=shop_dict["id"])
                        app.logger.info(
                            f"[WEBHOOK] Синхронизированы чаты для магазина {shop_dict['id']} после webhook: создано/обновлено {sync_result.get('synced_count', 0)} чатов"
                        )
                        ...
                        if chat:
                            app.logger.info(
                                f"[WEBHOOK] Новый чат {avito_chat_id} успешно создан после синхронизации (БД ID: {chat['id']})"
                            )
                            ...
                            try:
                                ...
                                api = AvitoAPI(
                                    client_id=shop_dict["client_id"],
                                    client_secret=shop_dict["client_secret"],
                                )
                                service = MessengerService(conn, api)
                                new_messages_count = service.sync_chat_messages(
                                    chat_id=chat["id"],
                                ...
                                app.logger.info(
                                    f"[WEBHOOK] Синхронизировано {new_messages_count} сообщений для нового чата {avito_chat_id}"
                                )
                            except Exception as msg_sync_err:
                                app.logger.error(
                                    f"[WEBHOOK] Ошибка синхронизации сообщений для нового чата: {msg_sync_err}",
                                    exc_info=True,
                                )
                    except Exception as sync_err:
                        app.logger.error(
                            f"[WEBHOOK] Ошибка синхронизации чатов: {sync_err}",
                            exc_info=True,
                        )
                elif chat:
                    ...
                    try:
                        ...
                        conn.commit()
                        app.logger.info(
                            f"[WEBHOOK] Синхронизировано {new_messages_count} новых сообщений для чата {avito_chat_id} (БД ID: {chat['id']})"
                        )
                        ...
                        app.logger.error(
                            f"[WEBHOOK] Ошибка синхронизации сообщений для чата {avito_chat_id}: {sync_err}",
                            exc_info=True,
                        )
                        conn.rollback()
                        ...
                        if avito_chat_id:
                            ...
                            if chat:
                                ...
                                try:
                                    sync_result = sync_chats_from_avito(shop_id=shop_dict["id"])
                                    app.logger.info(
                                        f"[WEBHOOK] Синхронизирован чат {avito_chat_id} для магазина {shop_dict['id']}: создано/обновлено {sync_result.get('synced_count', 0)} чатов"
                                    )
                                except Exception as chat_sync_err:
                                    app.logger.error(
                                        f"[WEBHOOK] Ошибка синхронизации чата {avito_chat_id}: {chat_sync_err}",
                                        exc_info=True,
                                    )
                            else:
                                # Чат не найден - возможно это новый чат, синхронизируем все чаты магазина
                                app.logger.info(
                                    f"[WEBHOOK] Чат {avito_chat_id} не найден в БД, синхронизируем все чаты магазина {shop_dict['id']} для создания нового"
                                )
                                try:
                                    sync_result = sync_chats_from_avito(shop_id=shop_dict["id"])
                                    app.logger.info(
                            ...
                            sync_result = sync_chats_from_avito(shop_id=shop_dict["id"])
                            app.logger.info(
                                f"[WEBHOOK] Синхронизированы все чаты для магазина {shop_dict['id']} после webhook: создано/обновлено {sync_result.get('synced_count', 0)} чатов"
                            )
                except Exception as sync_err:
                    app.logger.error(
                        f"[WEBHOOK] Ошибка синхронизации чатов после webhook: {sync_err}",
                        exc_info=True,
                    )
...
def get_manager_kpi(user_id):
    ...
    with get_connection() as conn:
        ...
        kpi_settings = conn.execute(
            "SELECT id, parameter_name, weight, min_value, penalty_amount, bonus_amount, created_at FROM kpi_settings"
        ).fetchall()
...
def get_system_settings():
    ...
    with get_connection() as conn:
        settings = conn.execute(
            "SELECT id, setting_key, setting_value, setting_type, description, updated_at FROM system_settings"
        ).fetchall()
    ...
    with get_connection() as conn:
        ...
        # Clients export
            ...
            clients = conn.execute(
                "SELECT id, name, phone, email, notes, total_orders, total_spent, is_blacklisted, created_at, updated_at FROM clients ORDER BY created_at DESC"
            ).fetchall()
        ...
        # Deliveries export
            ...
            # Экспорт доставок: админ видит все, менеджер — свои
            has_name_cols = check_name_columns(conn)
            if session.get("user_role") == "admin":
                if has_name_cols:
                    deliveries = conn.execute(
...
def get_notifications():
    ...
    # Оптимизированный запрос для получения статистики уведомлений одним запросом
    user_role = session.get("user_role")
    user_id = session.get("user_id")
    ...
    with get_connection() as conn:
        if user_role == "admin":
            stats = conn.execute(
                """
                SELECT 
                    (SELECT COUNT(*) FROM avito_chats WHERE priority = 'urgent' AND status != 'completed') as urgent_chats,
                    (SELECT COUNT(*) FROM avito_chats WHERE unread_count > 0 AND status != 'completed') as unread_chats
            """
            ).fetchone()
            ...
            stats = conn.execute(
                """
                SELECT 
                    (SELECT COUNT(*) FROM avito_chats c
                     JOIN avito_shops s ON c.shop_id = s.id
                     JOIN manager_assignments ma ON s.id = ma.shop_id
                     WHERE ma.manager_id = ? AND c.priority = 'urgent' AND c.status != 'completed') as urgent_chats,
                    (SELECT COUNT(*) FROM avito_chats c
                     JOIN avito_shops s ON c.shop_id = s.id
                     JOIN manager_assignments ma ON s.id = ma.shop_id
                     WHERE ma.manager_id = ? AND c.unread_count > 0 AND c.status != 'completed') as unread_chats
            """,
                (user_id, user_id),
            ).fetchone()
            urgent_chats = stats["urgent_chats"] or 0
        ...
        notifications.append(
            {
                ...
                "message": "Новые сообщения требуют ответа",
                "count": unread_chats,
            }
        )
...
def create_manager():
    ...
    try:
        ...
        if role == "admin" and user_role != "super_admin":
            app.logger.warning("[CREATE USER] Попытка создать админа обычным админом")
            return jsonify({"error": "Only super admin can create admin accounts"}), 403

        if not username or not email:
            app.logger.warning(
                f"[CREATE USER] Не заполнены обязательные поля: username={bool(username)}, email={bool(email)}"
            )
            return jsonify({"error": "Username and email are required"}), 400
...
def create_penalty():
    ...
    try:
        ...
        log_activity(
            ...
            "penalty",
            penalty_id,
            {"manager_id": manager_id, "amount": penalty_amount},
        )
...
def take_chat_from_pool(chat_id):
    ...
    try:
        ...
        if chat["status"] == "completed":
            ...
            return (
                jsonify({"error": "Cannot take completed chat", "code": "COMPLETED"}),
                400,
            )
        ...
        if chat["status"] == "blocked":
            ...
            return (
                jsonify({"error": "Cannot take blocked chat", "code": "BLOCKED"}),
                400,
            )
        ...
        # Логируем действие
        log_activity(user_id, "take_chat", f"Взят чат из пула ID: {chat_id}", "chat", chat_id)
...
def return_chat_to_pool(chat_id):
    ...
    try:
        ...
        if assigned_to is None:
            ...
            return (
                jsonify({"error": "Chat is already in pool", "code": "ALREADY_IN_POOL"}),
                400,
            )
        ...
        if user_role != "admin" and assigned_to != user_id:
            app.logger.warning(
                f"[RETURN CHAT] Попытка вернуть чужой чат {chat_id} (назначен {assigned_to}, пытается {user_id})"
            )
            return (
                jsonify(
                    {
        ...
        try:
            log_activity(
                user_id,
                "return_chat",
                f"Возвращен чат в пул ID: {chat_id}",
                "chat",
                chat_id,
            )
        ...
        return (
            jsonify({"success": True, "message": "Chat returned to pool successfully"}),
            200,
        )
    except Exception as e:
    ...
    try:
        ...
        try:
            log_activity(
                user_id,
                "return_all_chats",
                f"Возвращено {returned_count} чатов в пул",
                "chat",
                None,
            )
        except Exception as log_error:
        ...
        conn.commit()
        app.logger.info(
            f"[RETURN ALL CHATS] Возвращено {returned_count} чатов в пул пользователем {user_id} ({user_role})"
        )
        return (
            jsonify(
                {
        ...
        app.logger.error(
            f"[RETURN ALL CHATS] Ошибка при возврате всех чатов: {str(e)}",
            exc_info=True,
        )
        return jsonify({"error": str(e), "code": "INTERNAL_ERROR"}), 500
    ...
    try:
        ...
        if delivery:
            log_activity(
                session["user_id"],
                "move_delivery",
                f"Перенесена доставка на статус: {new_status}",
                "delivery",
                delivery_id,
            )
...
def delete_delivery(delivery_id):
    ...
    try:
        delivery = conn.execute(
            "SELECT id, chat_id, manager_id, delivery_status, address, tracking_number, notes, created_at, updated_at FROM deliveries WHERE id = ?",
            (delivery_id,),
        ).fetchone()
        if not delivery:
            # Соединение глобальное, не закрываем
        ...
        log_activity(
            session["user_id"],
            "delete_delivery",
            f"Удалена доставка ID: {delivery_id}",
            "delivery",
            delivery_id,
        )
...
def start_background_sync():
    ...
    def sync_worker():
        ...
        while True:
            try:
                # Проверяем, что БД инициализирована перед синхронизацией
                if not is_database_initialized():
                    app.logger.warning("[AUTO SYNC] База данных не инициализирована, пропускаем синхронизацию")
...
def register_webhooks_for_all_shops():
    ...
    # Проверяем, что БД инициализирована
    if not is_database_initialized():
        app.logger.warning("[WEBHOOK REGISTRATION] База данных не инициализирована, пропускаем регистрацию вебхуков")
        return
        ...
        for shop in shops:
            try:
                ...
                if shop["webhook_registered"]:
                    app.logger.debug(
                        f"[WEBHOOK REGISTRATION] Магазин {shop['name']} уже имеет зарегистрированный вебхук"
                    )
                    continue
                ...
                if webhook_result:
                    ...
                    conn.commit()
                    registered += 1
                    app.logger.info(f"[WEBHOOK REGISTRATION] [OK] Вебхук зарегистрирован для магазина {shop['name']}")
                else:
                    failed += 1
                    app.logger.warning(
                        f"[WEBHOOK REGISTRATION] [WARNING] Не удалось зарегистрировать вебхук для магазина {shop['name']}"
                    )

            except Exception as e:
                failed += 1
                app.logger.error(
                    f"[WEBHOOK REGISTRATION] Ошибка регистрации вебхука для магазина {shop['name']}: {e}",
                    exc_info=True,
                )
...
if not hasattr(app, "_background_tasks_started"):
    try:
        ...
        # Проверяем, что БД инициализирована перед запуском фоновых задач
        if is_database_initialized():
            ...
            app.logger.warning("[INIT] База данных не инициализирована, фоновые задачи не запущены")
    except Exception as e:
