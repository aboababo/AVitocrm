...
class AvitoPublicParser:
    ...
    def search_listings(
        ...
        """
        logger.info("[AVITO PARSER] ========== НАЧАЛО ПОИСКА ОБЪЯВЛЕНИЙ ==========")
        logger.info(
            f"[AVITO PARSER] Параметры поиска: query='{query}', category_id={category_id}, "
        ...
        try:
            ...
            logger.info("[AVITO PARSER] Отправка HTTP GET запроса к Avito...")
            ...
            logger.info(
                f"[AVITO PARSER] HTTP запрос выполнен за {request_time:.2f} сек. " f"Статус: {response.status_code}"
            )
            ...
            logger.info("[AVITO PARSER] Начало парсинга HTML...")
            ...
            if listings_count > 0:
                logger.debug("[AVITO PARSER] Примеры найденных объявлений:")
                for idx, listing in enumerate(listings[:5], 1):  # Первые 5
                    logger.debug(
                        f"[AVITO PARSER]   {idx}. ID={listing.get('listing_id', 'N/A')}, "
            ...
            logger.info("[AVITO PARSER] ========== ПОИСК ЗАВЕРШЕН УСПЕШНО ==========")
            ...
            logger.error(
                f"[AVITO PARSER] КРИТИЧЕСКАЯ ОШИБКА при поиске объявлений: {str(e)}",
                exc_info=True,
            )
            logger.error(f"[AVITO PARSER] URL запроса: {url if 'url' in locals() else 'N/A'}")
            raise
    ...
    def _parse_listings_from_html(self, soup: BeautifulSoup, limit: int) -> List[Dict]:
        ...
        try:
            ...
            logger.debug("[AVITO PARSER] Вариант 1: Поиск по data-marker='item'...")
            ...
            if not items:
                logger.debug("[AVITO PARSER] Вариант 2: Поиск по классам 'item|listing|ad'...")
            ...
            if not items:
                logger.debug("[AVITO PARSER] Вариант 3: Поиск по ссылкам с паттерном '/.*/\\d+$'...")
                items = soup.find_all("a", href=re.compile(r"/.*/\d+$"))
                logger.debug(f"[AVITO PARSER] Вариант 3: найдено {len(items)} элементов")

            total_items = len(items)
            logger.info(
                f"[AVITO PARSER] Всего найдено потенциальных элементов: {total_items}, "
                f"будет обработано: {min(total_items, limit)}"
            )

            if total_items == 0:
                logger.warning("[AVITO PARSER] Не найдено элементов для парсинга через HTML")
            ...
            for idx, item in enumerate(items[:limit], 1):
                try:
                    ...
                    if listing:
                        ...
                        successful_parses += 1
                        logger.debug(
                            f"[AVITO PARSER] Элемент {idx} успешно распарсен: " f"ID={listing.get('listing_id', 'N/A')}"
                        )
                    else:
                        failed_parses += 1
            ...
            logger.info(
                f"[AVITO PARSER] Парсинг HTML завершен: успешно={successful_parses}, "
            ...
            if not listings:
                logger.warning(
                    "[AVITO PARSER] Не найдено объявлений через HTML парсинг, " "пробуем парсить JSON из скриптов..."
                )
            ...
            logger.error(
                f"[AVITO PARSER] КРИТИЧЕСКАЯ ОШИБКА при парсинге HTML: {str(e)}",
                exc_info=True,
            )
    ...
    def _parse_single_listing(self, item) -> Optional[Dict]:
        ...
        try:
            ...
            # Ищем ссылку
            logger.debug("[AVITO PARSER] Поиск ссылки в элементе...")
            ...
            if link_elem:
                ...
                if match:
                    ...
                    logger.debug("[AVITO PARSER] Не удалось извлечь listing_id из URL")
            else:
                logger.debug("[AVITO PARSER] Ссылка не найдена")

            # Ищем заголовок
            logger.debug("[AVITO PARSER] Поиск заголовка...")
            ...
            if title_elem:
                ...
                logger.debug("[AVITO PARSER] Заголовок не найден")

            # Ищем цену
            logger.debug("[AVITO PARSER] Поиск цены...")
            ...
            if price_elem:
                ...
                logger.debug("[AVITO PARSER] Цена не найдена")

            # Ищем изображение
            logger.debug("[AVITO PARSER] Поиск изображения...")
            img_elem = item.find("img", src=True)
            if img_elem:
                listing["image_url"] = img_elem.get("src", "")
                logger.debug(f"[AVITO PARSER] Найдено изображение: {listing['image_url'][:80]}")
            else:
                logger.debug("[AVITO PARSER] Изображение не найдено")

            # Ищем локацию
            logger.debug("[AVITO PARSER] Поиск локации...")
            ...
            if location_elem:
                ...
                logger.debug("[AVITO PARSER] Локация не найдена")

            # Ищем описание
            logger.debug("[AVITO PARSER] Поиск описания...")
            ...
            if desc_elem:
                ...
                logger.debug("[AVITO PARSER] Описание не найдено")
            ...
            if has_url and has_title:
                logger.debug("[AVITO PARSER] Объявление валидно, возвращаем результат")
                return listing
            else:
                logger.debug("[AVITO PARSER] Объявление не валидно (нет обязательных полей)")
    ...
    def _parse_listings_from_json(self, soup: BeautifulSoup, limit: int) -> List[Dict]:
        ...
        try:
            ...
            logger.debug("[AVITO PARSER] Поиск script тегов с type='application/json'...")
                    ...
                    for item_idx, item in enumerate(items[:limit], 1):
                        try:
                            ...
                            if listing.get("url"):
                                ...
                                valid_items += 1
                                logger.debug(
                                    f"[AVITO PARSER] Добавлено объявление из JSON: "
                                    f"ID={listing['listing_id']}, title='{listing['title'][:40]}'"
                                )
                        except Exception as item_error:
                            logger.debug(
                                f"[AVITO PARSER] Ошибка обработки элемента {item_idx} из скрипта {script_idx}: {str(item_error)}"
                            )
                            continue
            ...
            logger.info(
                f"[AVITO PARSER] JSON парсинг завершен: обработано скриптов={processed_scripts}, "
                f"ошибок={failed_scripts}, найдено объявлений={len(listings)}"
            )

        except Exception as e:
            logger.error(
                f"[AVITO PARSER] КРИТИЧЕСКАЯ ОШИБКА при парсинге JSON: {str(e)}",
                exc_info=True,
            )
            ...
            try:
                ...
                if response.status_code == 429:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"[AVITO PARSER] Rate limit (429), ожидание {wait_time} сек... (попытка {attempt + 1}/{max_retries})"
                        )
                ...
                if price_elem and not details["price"]:
                    price_text = (
                        ...
                        else str(price_elem.get("content", ""))
                    )
                    # Извлекаем число из текста (убираем все кроме цифр)
                    price_match = re.search(
                        r"(\d[\d\s]*)",
                        price_text.replace(" ", "").replace("\xa0", "").replace(",", ""),
                    )
                    if price_match:
                        try:
                ...
                if not desc_elem:
                    desc_elem = soup.find(
                        "div",
                        {"data-marker": re.compile(r"item-description|description")},
                    )
                if not desc_elem:
                                    ...
                                    if isinstance(images_data, list):
                                        for img_data in images_data:
                                            if isinstance(img_data, dict):
                                                img_url = (
                                                    img_data.get("url") or img_data.get("src") or img_data.get("full")
                                                )
                                    ...
                                    if isinstance(photo_data, list):
                                        for photo in photo_data:
                                            img_url = (
                                                ...
                                                else str(photo) if photo else None
                                            )
                                            if img_url and img_url not in details["images"]:
                                                details["images"].append(img_url)
                        except:
                ...
                logger.info(
                    f"[AVITO PARSER] Парсинг завершен: title={bool(details['title'])}, price={bool(details['price'])}, "
                ...
                if http_error.response and http_error.response.status_code == 429:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(
                            f"[AVITO PARSER] HTTP ошибка {http_error.response.status_code if http_error.response else 'unknown'}, повтор через {wait_time} сек..."
                        )
                        time.sleep(wait_time)
                        continue
                    else:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(
                        f"[AVITO PARSER] ❌ Ошибка получения деталей объявления {url} после {max_retries} попыток: {e}",
                        exc_info=True,
                    )
                    raise
