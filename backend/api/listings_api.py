...
def search_listings():
    ...
    logger.info(
        f"[LISTINGS API] Параметры поиска: query='{query}', category_id={category_id}, "
    ...
    try:
        with get_connection() as conn:
            logger.debug("[LISTINGS API] Подключение к БД установлено")

            service = ListingsService(conn)
            logger.debug("[LISTINGS API] ListingsService создан")

        # Поиск
        logger.info("[LISTINGS API] Вызов service.search_public_listings()...")
        import time
        ...
        logger.info(
            f"[LISTINGS API] Поиск завершен за {elapsed_time:.2f} сек. "
        ...
        param_id = None
            ...
            if save_results:
            logger.info("[LISTINGS API] Сохранение результатов включено. Начало сохранения...")

            if results.get("listings"):
                logger.debug("[LISTINGS API] Сохранение параметров поиска...")
                ...
                for idx, listing in enumerate(results["listings"], 1):
                    ...
                    logger.debug(
                        f"[LISTINGS API] Сохранение объявления {idx}/{listings_count}: "
                ...
                logger.info(
                    f"[LISTINGS API] Сохранение завершено за {save_elapsed:.2f} сек. "
                    f"Сохранено: {saved_count}/{listings_count} объявлений"
                )
            else:
                logger.warning("[LISTINGS API] Нет объявлений для сохранения")
        else:
            logger.debug("[LISTINGS API] Сохранение результатов отключено")
        ...
        logger.info(
            f"[LISTINGS API] Поиск завершен успешно. Возврат ответа: "
        ...
        logger.error(
            f"[LISTINGS API] КРИТИЧЕСКАЯ ОШИБКА при поиске объявлений: {str(e)}",
            exc_info=True,
        )
        return jsonify({"error": str(e), "listings": [], "total": 0, "saved": 0}), 500
    ...
    try:
        with get_connection() as conn:
            ...
            listings, total = service.get_saved_listings(
                status=status, assigned_manager_id=manager_id, limit=limit, offset=offset
            )
            ...
            logger.info(
                f"[LISTINGS API] Запрос выполнен за {elapsed_time:.2f} сек. "
        ...
        return jsonify({"error": str(e), "listings": [], "total": 0}), 500
...
def update_listing(listing_id):
    ...
    with get_connection() as conn:
        ...
        if session.get("user_role") == "manager" and listing["assigned_manager_id"] not in (
            None,
            session.get("user_id"),
        ):
