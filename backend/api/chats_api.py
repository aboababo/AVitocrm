...
def get_messages(chat_id):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"[API/MESSAGES] Ошибка получения данных чата {chat_id}: {e}",
                exc_info=True,
            )
            return (
                jsonify({"error": f"Database error: {str(e)}", "code": "DB_ERROR"}),
                500,
            )
        ...
        logger.info(
            f"[API/MESSAGES] Запрос сообщений для чата {chat_id}, sync={sync}, client_id={bool(chat.get('client_id'))}, client_secret={bool(chat.get('client_secret'))}, user_id={chat.get('user_id')}"
        )
        ...
        if sync and chat.get("client_id") and chat.get("client_secret") and chat.get("user_id"):
            try:
                logger.info(
                    f"[API/MESSAGES] Начинаем синхронизацию сообщений для чата {chat_id}, user_id={chat['user_id']}, avito_chat_id={chat.get('chat_id')}"
                )
            ...
            if sync:
                logger.warning(
                    f"[API/MESSAGES] Синхронизация запрошена, но не выполнена: client_id={bool(chat.get('client_id'))}, client_secret={bool(chat.get('client_secret'))}, user_id={bool(chat.get('user_id'))}"
                )
            service = MessengerService(conn, None)
        ...
        try:
            ...
            logger.error(
                f"[API/MESSAGES] Ошибка получения сообщений для чата {chat_id}: {msg_error}",
                exc_info=True,
            )
            import traceback
        ...
        logger.error(
            f"[API/MESSAGES] Критическая ошибка при получении сообщений для чата {chat_id}: {e}",
            exc_info=True,
        )
        import traceback
...
def send_message(chat_id):
    ...
    if not message_text and not attachments:
        return (
            jsonify({"error": "Either message text or attachments are required"}),
            400,
        )
    ...
    if attachments:
        for i, attachment in enumerate(attachments):
            ...
            if "id" not in attachment:
                return (
                    jsonify({"error": f'Attachment {i} must have an "id" field'}),
                    400,
                )
    ...
    logger.info("[SEND MESSAGE] ========== НАЧАЛО ОТПРАВКИ СООБЩЕНИЯ ==========")
    ...
    if attachments:
        try:
            logger.info("[SEND MESSAGE] Отправка через api.send_message с attachments")
            ...
            logger.error(
                f"[SEND MESSAGE] Ошибка отправки сообщения с вложениями: {e}",
                exc_info=True,
            )
            # Соединение глобальное, не закрываем
        ...
        logger.info("[SEND MESSAGE] Отправка через service.send_message (без attachments)")
        ...
        if success:
            ...
            logger.error(
                f"[SEND MESSAGE] ❌ Ошибка отправки через service.send_message для чата {chat_id}: {error_msg}"
            )
...
def sync_chats():
    ...
    try:
        from tasks import RQ_AVAILABLE, enqueue_sync_all_chats
        ...
        if RQ_AVAILABLE:
            ...
            if hasattr(job, "id"):
                ...
                return (
                    jsonify({"success": True, "status": "completed", "result": job}),
                    200,
                )
        else:
...
def extract_all_product_urls():
    ...
    try:
        ...
        logger.info(
            f"[EXTRACT ALL] Найдено {len(chats_without_url)} чатов без product_url (всего без URL: {total_count}, offset: {offset}, limit: {limit})"
        )
            ...
            try:
                ...
                if chat["client_id"] and chat["client_secret"] and chat["user_id"]:
                    try:
                        ...
                        chat_details = api.get_chat_by_id(user_id=chat["user_id"], chat_id=avito_chat_id)
                            ...
                            if isinstance(detail_context, dict):
                                ...
                                if isinstance(detail_item, dict):
                                    ...
                                    if detail_url:
                                        ...
                                        shop_url_part = (
                                            chat.get("shop_url", "").split("/")[-1] if chat.get("shop_url") else ""
                                        )
                                        if shop_url_part:
                        ...
                        logger.warning(
                            f"[EXTRACT ALL] Ошибка API для чата {chat_id} (avito_chat_id={avito_chat_id}): {api_error}"
                        )
        ...
        logger.info(
            f"[EXTRACT ALL] Завершено: обработано {results['total']}, найдено {results['extracted']}, ошибок {results['errors']}, осталось: {results.get('total_without_url', 0) - offset - len(chats_without_url)}"
        )
        ...
        if results.get("has_more"):
            ...
            response_data["message"] = (
                f"Обработано {len(chats_without_url)} из {total_count} чатов. Для продолжения отправьте запрос с offset={offset + limit}"
            )
...
def extract_all_product_urls_internal():
    ...
    if expected_api_key and api_key != expected_api_key:
        logger.warning("[EXTRACT ALL INTERNAL] Неверный API ключ")
    ...
    try:
        ...
        logger.info(
            f"[EXTRACT ALL INTERNAL] Найдено {len(chats_without_url)} чатов без product_url (всего без URL: {total_count}, offset: {offset}, limit: {limit})"
        )
        ...
        for idx, chat_row in enumerate(chats_without_url):
            ...
            try:
                ...
                if chat["client_id"] and chat["client_secret"] and chat["user_id"]:
                    try:
                        ...
                        if idx < 3:
                            ...
                            if isinstance(chat_details, dict):
                                logger.info(
                                    f"[EXTRACT ALL INTERNAL] Чат {chat_id}: ключи в ответе: {list(chat_details.keys())[:20]}"
                                )
                                if "context" in chat_details:
                                    logger.info(
                                        f"[EXTRACT ALL INTERNAL] Чат {chat_id}: context тип={type(chat_details.get('context'))}"
                                    )
                                    if isinstance(chat_details.get("context"), dict):
                                        logger.info(
                                            f"[EXTRACT ALL INTERNAL] Чат {chat_id}: context ключи: {list(chat_details.get('context', {}).keys())}"
                                        )
                        ...
                        if isinstance(chat_details, dict):
                            ...
                            if isinstance(detail_context, dict):
                                ...
                                if idx < 3:
                                    logger.info(
                                        f"[EXTRACT ALL INTERNAL] Чат {chat_id}: detail_item тип={type(detail_item)}, значение={str(detail_item)[:200] if detail_item else 'None'}"
                                    )
                                ...
                                if isinstance(detail_item, dict):
                                    ...
                                    if idx < 3:
                                        logger.info(
                                            f"[EXTRACT ALL INTERNAL] Чат {chat_id}: detail_item_id={detail_item_id}, detail_url={detail_url}"
                                        )
                            ...
                            if not product_url:
                                ...
                                if idx < 3:
                                    logger.info(
                                        f"[EXTRACT ALL INTERNAL] Чат {chat_id}: проверка прямых полей, product_url={product_url}"
                                    )
                    except Exception as api_error:
                        logger.warning(
                            f"[EXTRACT ALL INTERNAL] Ошибка API для чата {chat_id} (avito_chat_id={avito_chat_id}): {api_error}"
                        )
                ...
                logger.error(
                    f"[EXTRACT ALL INTERNAL] Ошибка для чата {chat_id}: {e}",
                    exc_info=True,
                )
                results["errors"] += 1
        ...
        logger.info(
            f"[EXTRACT ALL INTERNAL] Завершено: обработано {results['total']}, найдено {results['extracted']}, ошибок {results['errors']}"
        )
...
def extract_product_url_from_messages(chat_id):
    ...
    try:
        ...
        if chat_dict.get("existing_product_url"):
            logger.info(
                f"[EXTRACT PRODUCT URL] У чата {chat_id} уже есть product_url: {chat_dict['existing_product_url']}"
            )
            return (
                jsonify(
                    {
        ...
        if chat_dict.get("client_id") and chat_dict.get("client_secret") and chat_dict.get("user_id"):
            ...
            logger.info("[EXTRACT PRODUCT URL] Пытаемся получить данные через Avito API...")
            try:
                api = AvitoAPI(
                ...
                app_logger.info(
                    f"[EXTRACT PRODUCT URL] API вернул данные для чата {chat_id}. Тип: {type(chat_details)}"
                )
                ...
                if isinstance(chat_details, dict):
                    ...
                    if "context" in chat_details:
                        ...
                        app_logger.warning(
                            f"[EXTRACT PRODUCT URL] В ответе API для чата {chat_id} НЕТ поля 'context'! Это означает, что чат не связан с конкретным объявлением."
                        )
                        logger.warning("[EXTRACT PRODUCT URL] В ответе API НЕТ поля 'context'!")
                else:
                    app_logger.warning(
                        f"[EXTRACT PRODUCT URL] API вернул не словарь для чата {chat_id}: {chat_details}"
                    )
                    ...
                    if isinstance(detail_context, dict):
                        ...
                        if isinstance(detail_item, dict):
                            ...
                            if detail_url:
                                ...
                                app_logger.info(
                                    f"[EXTRACT PRODUCT URL] ✅ Найден через API context.value.url для чата {chat_id}: {product_url}"
                                )
                                logger.info(
                                    f"[EXTRACT PRODUCT URL] ✅ Найден через API context.value.url: {product_url}"
                                )
                            elif detail_item_id:
                                item_id_str = str(detail_item_id)
                                shop_url_part = (
            ...
            if (
                not product_url
                and chat_dict.get("client_id")
                and chat_dict.get("client_secret")
                and chat_dict.get("user_id")
            ):
                try:
                    ...
                    for page in range(max_pages):
                        ...
                        if isinstance(chats_list, dict):
                            ...
                            chats_array = (
                                chats_list.get("chats")
                                or chats_list.get("items")
                                or chats_list.get("data", {}).get("chats")
                                or []
                            )
                        ...
                        for idx, chat_item in enumerate(chats_array):
                            if isinstance(chat_item, dict):
                                ...
                                item_id = (
                                    chat_item.get("id")
                                    or chat_item.get("chat_id")
                                    or chat_item.get("value", {}).get("id")
                                )

                                # Сравниваем ID (с учетом разных форматов)
                                if item_id and (str(item_id) == str(avito_chat_id) or item_id == avito_chat_id):
                                    target_chat = chat_item
                                    logger.info(
                                        f"[EXTRACT PRODUCT URL] ✅ Найден чат в списке get_chats (страница {page + 1})"
                                    )
                                    break
                    ...
                    if target_chat:
                        ...
                        if isinstance(target_context, dict):
                            # ВАЖНО: Avito API v3 возвращает context.value, а не context.item!
                            context_item = (
                                target_context.get("value")
                                or target_context.get("item")
                                or target_context.get("listing")
                                or target_context.get("ad", {})
                            )

                            if isinstance(context_item, dict):
                                context_item_id = context_item.get("id")
                                context_url = (
                                    context_item.get("url")
                                    or context_item.get("link")
                                    or context_item.get("href")
                                    or context_item.get("value")
                                    or context_item.get("uri")
                                )
                                ...
                                if context_url:
                                    ...
                                    shop_url_part = (
                                        chat_dict.get("shop_url", "").split("/")[-1]
                                        if chat_dict.get("shop_url")
                                        else ""
                                    )
                                    if shop_url_part:
                                        product_url = f"https://www.avito.ru/{shop_url_part}/items/{item_id_str}"
                        ...
                        if chats_array and len(chats_array) > 0:
                            ...
                            for idx, sample_chat in enumerate(chats_array[:3]):  # Первые 3 чата
                                if isinstance(sample_chat, dict):
                                    sample_id = (
                                        sample_chat.get("id")
                                        or sample_chat.get("chat_id")
                                        or sample_chat.get("value", {}).get("id")
                                    )
                                    sample_ids.append(str(sample_id))
                            logger.warning(
                                f"[EXTRACT PRODUCT URL] ⚠️ Чат {avito_chat_id} не найден. Примеры ID: {', '.join(sample_ids)}"
                            )
                except Exception as get_chats_error:
                    logger.debug(f"[EXTRACT PRODUCT URL] Ошибка get_chats: {get_chats_error}")
        else:
            logger.warning(
                "[EXTRACT PRODUCT URL] Нет credentials для API запроса (client_id, client_secret или user_id отсутствуют)"
            )
            ...
            jsonify(
                {
                    ...
                    "debug_info": {
                        "has_api_credentials": bool(
                            chat_dict.get("client_id") and chat_dict.get("client_secret") and chat_dict.get("user_id")
                        ),
