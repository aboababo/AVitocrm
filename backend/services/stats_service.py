...
class StatsService:
    ...
    def get_dashboard_stats(self) -> Dict:
        ...
        chats_active = self.conn.execute(
            "SELECT COUNT(*) as count FROM avito_chats WHERE status = 'active'"
        ).fetchone()["count"]
        chats_pool = self.conn.execute(
        ...
        listings_new = self.conn.execute(
            "SELECT COUNT(*) as count FROM avito_listings WHERE status = 'new'"
        ).fetchone()["count"]
        listings_contacted = self.conn.execute(
            "SELECT COUNT(*) as count FROM avito_listings WHERE status = 'contacted'"
        ).fetchone()["count"]

        # Доставки
        deliveries_total = self.conn.execute("SELECT COUNT(*) as count FROM deliveries").fetchone()["count"]
        deliveries_active = self.conn.execute(
            "SELECT COUNT(*) as count FROM deliveries WHERE delivery_status IN ('pending', 'shipped')"
        ).fetchone()["count"]

        return {
            "chats": {"total": chats_total, "active": chats_active, "pool": chats_pool},
            ...
            "deliveries": {"total": deliveries_total, "active": deliveries_active},
        }
