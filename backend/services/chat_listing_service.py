...
class ChatListingService:
    ...
    @staticmethod
    def get_chat_listing_info(
        chat_id: int, try_extract_from_api: bool = True
        ...
        with get_connection() as conn:
            ...
            if listing_data_str:
                try:
                    ...
                    if isinstance(listing_data, dict) and (listing_data.get("title") or listing_data.get("url")):
                        ...
                        logger.info(
                            f"[CHAT LISTING SERVICE] ✅ Найдены сохраненные данные объявления из синхронизации: title={bool(listing_data.get('title'))}, price_string={bool(listing_data.get('price_string'))}, images={bool(listing_data.get('images'))}, url={bool(listing_data.get('url'))}"
                        )
                    else:
                        logger.warning(
                            f"[CHAT LISTING SERVICE] listing_data есть, но неполные: title={bool(listing_data.get('title'))}, url={bool(listing_data.get('url'))}"
                        )
                except Exception as e:
                    logger.warning(f"[CHAT LISTING SERVICE] Ошибка парсинга listing_data: {e}")
            else:
                logger.info("[CHAT LISTING SERVICE] listing_data отсутствует в БД, будем загружать из API")
            ...
            logger.error(
                f"[CHAT LISTING SERVICE] Ошибка получения информации о чате {chat_id}: {e}",
                exc_info=True,
            )
            return None, None, None

    @staticmethod
    def _extract_listing_from_chat_context(
        client_id: str, client_secret: str, avito_user_id: str, avito_chat_id: str
    ) -> Optional[Dict]:
        ...
        try:
            ...
            if not isinstance(item_data, dict) or not item_data:
                logger.warning(
                    f"[CHAT LISTING SERVICE] item_data пустой или не dict. context keys: {list(context.keys())}"
                )
                return None
            ...
            if isinstance(images_data, dict):
                ...
                if isinstance(main_image, dict):
                    size_urls = [
                        ((int(m.group(1)) * int(m.group(2)), url) if (m := re.search(r"(\d+)x(\d+)", k)) else (0, url))
                        for k, url in main_image.items()
                        if url
                    ]
            ...
            if has_title or has_url:
                logger.info(
                    f"[CHAT LISTING SERVICE] ✅ Данные извлечены: title={has_title}, url={has_url}, price={bool(listing_data.get('price'))}, images={len(images_list)}"
                )
                return listing_data
            else:
                logger.warning("[CHAT LISTING SERVICE] Данные неполные: нет title и url")
                return None
        except Exception as e:
            logger.error(
                f"[CHAT LISTING SERVICE] Ошибка при извлечении данных из чата: {e}",
                exc_info=True,
            )
            return None

    @staticmethod
    def _extract_product_url_from_api(
        client_id: str,
        client_secret: str,
        avito_user_id: str,
        avito_chat_id: str,
        ...
        try:
            ...
            try:
                ...
                if isinstance(chat_details, dict):
                    value = (
                        chat_details.get("context", {}).get("value", {})
                    ...
                    if isinstance(value, dict):
                        ...
                        if url:
                            return (
                                f"https://www.avito.ru{url}"
                                if url.startswith("/")
                                else (f"https://www.avito.ru/{url}" if not url.startswith("http") else url)
                            )
            except Exception as e:
                if "404" in str(e):
                    try:
                        ...
                        chats_array = (
                            ...
                            else chats_list if isinstance(chats_list, list) else []
                        )
                        ...
                        for chat_item in chats_array:
                            if isinstance(chat_item, dict):
                                ...
                                if item_id and str(item_id) == str(avito_chat_id):
                                    context_item = (
                                        (
                                            chat_item.get("context", {}).get("value")
                                            or chat_item.get("context", {}).get("item")
                                            or {}
                                        )
                                        if isinstance(chat_item.get("context"), dict)
                                        else {}
                                    )
    ...
    def get_listing_from_avito(
        ...
        if not avito_chat_id:
            logger.warning("[CHAT LISTING] avito_chat_id отсутствует")
            return {}
        ...
        try:
            ...
            if listing_info:
                logger.info(
                    f"[CHAT LISTING] Данные извлечены из API: title={bool(listing_info.get('title'))}, price={bool(listing_info.get('price'))}, images={len(listing_info.get('images', []))}"
                )
            else:
                logger.warning("[CHAT LISTING] API не вернул данные об объявлении")

            return listing_info or {}
        except Exception as e:
            logger.error(
                f"[CHAT LISTING] Ошибка при извлечении данных из API: {e}",
                exc_info=True,
            )
            return {}

    @staticmethod
    def get_chat_listing(
        chat_id: int, user_id: Optional[int] = None, user_role: Optional[str] = None
    ) -> Dict[str, Any]:
            ...
            if cached_listing:
                try:
                    ...
                    if listing_data_str:
                        ...
                        images = (
                            listing_data.get("images") or listing_data.get("photos") or listing_data.get("pictures") or []
                        )
        ...
        if saved_listing_data and isinstance(saved_listing_data, dict):
            logger.info("[CHAT LISTING] ✅ Найдены сохраненные данные из синхронизации (context.value)")
            logger.info(
                f"[CHAT LISTING] Данные: title={bool(saved_listing_data.get('title'))}, price_string={bool(saved_listing_data.get('price_string'))}, images={bool(saved_listing_data.get('images'))}, url={bool(saved_listing_data.get('url'))}"
            )
            ...
            if not listing_info.get("title") and not listing_info.get("url"):
                logger.warning("[CHAT LISTING] Сохраненные данные неполные (нет title и url), загружаем из API")
                listing_info = None
            else:
                logger.info(
                    f"[CHAT LISTING] ✅ Данные готовы: title={bool(listing_info.get('title'))}, price={bool(listing_info.get('price'))}, images={len(listing_info.get('images', []))}"
                )

        # Если нет сохраненных данных или они неполные, загружаем из API
        if not listing_info:
            logger.info("[CHAT LISTING] Загружаем данные из API чата...")
            try:
                ...
                if is_valid and avito_chat_id:
                    logger.info("[CHAT LISTING] OAuth ключи валидны, запрашиваем данные из API...")
                    ...
                    if listing_info:
                        logger.info(
                            f"[CHAT LISTING] Данные получены из API: title={bool(listing_info.get('title'))}, price={bool(listing_info.get('price'))}, images={len(listing_info.get('images', []))}"
                        )
                    else:
                        logger.warning("[CHAT LISTING] API вернул пустые данные")
                else:
                    if not is_valid:
                        logger.warning("[CHAT LISTING] OAuth ключи не валидны")
                    if not avito_chat_id:
                        logger.warning("[CHAT LISTING] avito_chat_id отсутствует")
            except Exception as api_error:
                logger.error(
                    f"[CHAT LISTING] Ошибка при загрузке из API: {api_error}",
                    exc_info=True,
                )
        ...
        if not listing_info or (
            isinstance(listing_info, dict) and not listing_info.get("title") and not listing_info.get("url")
        ):
            # Используем сохраненные данные из синхронизации (context.value)
            if saved_listing_data and isinstance(saved_listing_data, dict):
                logger.info("[CHAT LISTING] Используем сохраненные данные из синхронизации (context.value)")
                listing_info = saved_listing_data.copy()
                ...
                logger.info(
                    f"[CHAT LISTING] ✅ Данные из сохраненных: title={bool(listing_info.get('title'))}, price={bool(listing_info.get('price'))}, images={len(listing_info.get('images', []))}"
                )
            else:
                # Если нет сохраненных данных, возвращаем ошибку
                logger.warning(
                    f"[CHAT LISTING] Не удалось получить данные объявления. saved_listing_data={bool(saved_listing_data)}, listing_info={bool(listing_info)}"
                )
                return {
                    "success": False,
                    "error": "Failed to extract listing data from chat",
        ...
        try:
            ...
            logger.error(
                f"[CHAT LISTING] Ошибка нормализации данных: {normalize_error}",
                exc_info=True,
            )
        ...
        try:
            with get_connection() as conn:
                if has_title and has_price and has_images:
                    ...
                    cache_item_id = item_id or (
                        hashlib.md5(final_url.encode()).hexdigest()[:12] if final_url else str(chat_id)
                    )
    ...
    def normalize_listing_data(listing_info: Dict) -> Dict:
        ...
        normalized = {
            ...
            or listing_info.get("text")
            or listing_info.get("content")
            or listing_info.get("about")
            or "",
