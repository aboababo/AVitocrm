...
class ListingsService:
    ...
    def search_public_listings(
        ...
        """
        logger.info("[LISTINGS SERVICE] Начало поиска объявлений через публичный парсер")
        logger.debug(
            f"[LISTINGS SERVICE] Параметры: query='{query}', category_id={category_id}, "
        ...
        try:
            logger.debug("[LISTINGS SERVICE] Создание экземпляра AvitoPublicParser...")
            parser = AvitoPublicParser()
            logger.debug("[LISTINGS SERVICE] AvitoPublicParser создан успешно")

            import time

            start_time = time.time()

            logger.info("[LISTINGS SERVICE] Вызов parser.search_listings()...")
            ...
            logger.info(
                f"[LISTINGS SERVICE] Поиск завершен за {elapsed_time:.2f} сек. "
            ...
            if listings_count > 0:
                ...
                logger.debug("[LISTINGS SERVICE] Примеры найденных объявлений:")
                for idx, listing in enumerate(sample_listings, 1):
                    ...
                    logger.debug(
                        f"[LISTINGS SERVICE]   {idx}. ID={listing_id}, title='{title}', " f"price={price}, url={url}"
                    )

            if "error" in results:
                logger.warning(f"[LISTINGS SERVICE] В результатах есть ошибка: {results.get('error')}")

            logger.info("[LISTINGS SERVICE] Поиск завершен успешно")
            return results

        except Exception as e:
            logger.error(
                f"[LISTINGS SERVICE] КРИТИЧЕСКАЯ ОШИБКА при поиске объявлений: {str(e)}",
                exc_info=True,
            )
    ...
    def save_listing(self, listing_data: Dict, param_id: Optional[int] = None) -> int:
        ...
        logger.debug(
            f"[LISTINGS SERVICE] Сохранение объявления: listing_id={listing_id}, "
        ...
        try:
            ...
            logger.debug("[LISTINGS SERVICE] Объявление не найдено, создание новой записи...")
            ...
            self.conn.commit()
            logger.debug("[LISTINGS SERVICE] Изменения зафиксированы в БД")

            return new_id

        except Exception as e:
            logger.error(
                f"[LISTINGS SERVICE] ОШИБКА сохранения объявления listing_id={listing_id}: {str(e)}",
                exc_info=True,
            )
            return 0
    ...
    def save_search_params(self, params: Dict, user_id: int) -> int:
        ...
        try:
            ...
            logger.error(
                f"[LISTINGS SERVICE] ОШИБКА сохранения параметров поиска: {str(e)}",
                exc_info=True,
            )
            return 0
    ...
    def get_saved_listings(
        ...
        """
        logger.debug(
            f"[LISTINGS SERVICE] Получение сохраненных объявлений: status='{status}', "
        ...
        count_query = (
            "SELECT COUNT(*) as count FROM avito_listings WHERE 1=1"
        ...
        logger.info(
            f"[LISTINGS SERVICE] Запрос выполнен: получено {listings_count} объявлений, " f"всего в БД: {total}"
        )
