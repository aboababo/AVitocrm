...
def init_database():
    ...
    if cursor.fetchone()[0] == 0:
        # Импортируем функцию хеширования из auth.py
        try:
            from auth import hash_password
        except ImportError:
            # Fallback на старый метод если auth.py недоступен
        ...
        default_settings = [
            ...
            (
                "timer_urgent_minutes",
                "20",
                "number",
                'Время для "срочного" чата (минуты)',
            ),
            (
                "timer_critical_minutes",
                "60",
                "number",
                "Критическое время ответа (минуты)",
            ),
            (
                "notification_blink_interval",
                "1000",
                "number",
                "Интервал мигания уведомлений (мс)",
            ),
            ...
            (
                ...
                "json",
                "Видимость вкладок для менеджера",
            ),
        ]
        ...
        default_templates = [
            (
                "Приветствие",
                "Здравствуйте! Спасибо за интерес к нашему товару. Чем могу помочь?",
                "greeting",
                1,
            ),
            (
                "Уточнение наличия",
                "Да, товар в наличии. Могу ответить на ваши вопросы.",
                "response",
                1,
            ),
            (
                "Прощание",
                "Спасибо за обращение! Если возникнут вопросы, обращайтесь.",
                "closing",
                1,
            ),
        ]
        ...
        default_quick_replies = [
            ...
            (
                "/цена",
                "Цена актуальна. Могу предоставить дополнительную информацию.",
                1,
            ),
        ]
        ...
        test_chats = [
            ...
            (
                ...
                "Иван Петров",
                "+79161234567",
                "https://www.avito.ru/iphone",
                "Здравствуйте! Интересует iPhone 13. Есть в наличии?",
                "urgent",
                "active",
                1,
                25,
            ...
            (
                ...
                "Мария Сидорова",
                "+79167654321",
                "https://www.avito.ru/macbook",
                "Срочно нужен MacBook Pro! Цена актуальна?",
                "urgent",
                "active",
                2,
                35,
            ...
            (
                ...
                "Алексей Козлов",
                "+79169998877",
                "https://www.avito.ru/jacket",
                "Добрый день! Какой размер посоветуете?",
                "new",
                "active",
                0,
                5,
            ...
            (
                ...
                "Елена Васнецова",
                "+79165554433",
                "https://www.avito.ru/airpods",
                "Здравствуйте! AirPods Pro есть в наличии?",
                "new",
                "active",
                0,
                15,
            ...
            (
                ...
                "Дмитрий Орлов",
                "+79162223344",
                "https://www.avito.ru/shoes",
                "Спасибо! Жду доставку",
                "active",
                "active",
                0,
                120,
    ...
    indexes = [
        ...
        # Индексы для сообщений (дополнительные)
        "CREATE INDEX IF NOT EXISTS idx_messages_is_read ON avito_messages(is_read)",
        "CREATE INDEX IF NOT EXISTS idx_messages_chat_read ON avito_messages(chat_id, is_read)",
        # Индексы для чатов (дополнительные)
        "CREATE INDEX IF NOT EXISTS idx_chats_unread ON avito_chats(unread_count)",
        "CREATE INDEX IF NOT EXISTS idx_chats_manager_status ON avito_chats(assigned_manager_id, status)",
        "CREATE INDEX IF NOT EXISTS idx_chats_created_at ON avito_chats(created_at DESC)",
    ]
...
# Thread-local хранилище для соединений с базой данных
# Каждый поток имеет свое соединение, что решает проблему многопоточности
...
def get_db_connection():
    ...
    Использует thread-local хранилище для хранения соединений.
    Каждый поток получает свое собственное соединение, что решает
    проблему использования SQLite объектов между потоками.
    ...
    Настройки:
        ...
        - check_same_thread=False: Разрешает использование в разных потоках

    Важно:
        - Каждый поток получает свое соединение
        - Соединения автоматически закрываются при завершении потока
        - Соединение автоматически переподключается при ошибках

    Returns:
        sqlite3.Connection: Соединение с базой данных для текущего потока
    """
    ...
    if hasattr(_thread_local, "connection"):
        ...
        try:
            ...
            # Соединение закрыто или повреждено, удаляем его
            error_msg = str(e).lower()
    ...
    # Сохраняем соединение в thread-local хранилище
...
def safe_init_database():
    ...
    backup_file = os.path.join(
        backup_dir,
        f"osagaming_crm_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db",
    )
