...
def get_system_stats():
    ...
    with get_connection() as conn:
        ...
        active_chats = conn.execute('SELECT COUNT(*) as count FROM avito_chats WHERE status = "active"').fetchone()[
            "count"
        ]

        # Считаем срочные чаты (требующие немедленного ответа)
        urgent_chats = conn.execute('SELECT COUNT(*) as count FROM avito_chats WHERE priority = "urgent"').fetchone()[
            "count"
        ]
        ...
        avito_stats = {"total_messages": 0, "new_messages": 0, "responses": 0}
