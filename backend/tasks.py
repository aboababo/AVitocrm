    ...
    try:
        ...
        with get_connection() as conn:
            ...
            new_count = messenger_service.sync_chat_messages(
                chat_id=chat_id, user_id=user_id, avito_chat_id=avito_chat_id
            )
...
def enqueue_sync_chat_messages(chat_id: int, user_id: str, avito_chat_id: str):
    ...
    try:
        job = sync_queue.enqueue(sync_chat_messages_task, chat_id, user_id, avito_chat_id, job_timeout="2m")
...
def enqueue_webhook(webhook_data: Dict):
    ...
    try:
        job = default_queue.enqueue(process_webhook_task, webhook_data, job_timeout="1m")
        return job
    except Exception as e:
