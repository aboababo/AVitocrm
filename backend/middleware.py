...
RATE_LIMIT_CONFIG = {
    ...
    "/api/messages": {
        "requests": 300,
        "window": 60,
    },  # 300 запросов в минуту для сообщений
    "/api/sync": {
        "requests": 10,
        "window": 60,
    },  # 10 запросов в минуту для синхронизации
}
            ...
            with _rate_limit_lock:
                if client_id in _rate_limit_storage:
                    _rate_limit_storage[client_id] = [
                        req_time for req_time in _rate_limit_storage[client_id] if now - req_time < time_window
                    ]
                else:
                    _rate_limit_storage[client_id] = []
                ...
                if len(_rate_limit_storage[client_id]) >= req_limit:
                    logger.warning(
                        f"Rate limit exceeded for {client_id} on {config_key}: {len(_rate_limit_storage[client_id])}/{req_limit}"
                    )
                    ...
                    if request.path.startswith("/login") and request.method == "GET":
                        ...
                        return (
                            render_template(
                                "login.html",
                                error=f"Превышен лимит запросов. Подождите {time_window} секунд.",
                            ),
                            429,
                        )
                    return (
...
def log_request(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            logger.error(
                f"Error in {request.method} {request.path}: {str(e)} - {duration:.2f}ms",
                exc_info=True,
            )
            raise
